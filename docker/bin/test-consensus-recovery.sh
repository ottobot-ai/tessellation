...
check_ordinals() {
    echo "=== Node Ordinals ==="
    for container in $(get_gl0_containers); do
        local ordinal=$(docker exec "$container" curl -s http://localhost:9000/global-snapshots/latest 2>/dev/null | jq -r '.value.ordinal // "N/A"')
        local state=$(docker exec "$container" curl -s http://localhost:9000/node/state 2>/dev/null | tr -d '"')
        echo "$container: ordinal=$ordinal state=$state"
    done
}
