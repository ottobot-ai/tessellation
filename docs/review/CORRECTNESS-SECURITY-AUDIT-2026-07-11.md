...
| SMT-02 | CRITICAL | IMPLEMENTATION/DESIGN | exact branch/base identity | `MptOverlay.scala:37-40,762-805,828-950,1011-1030`; `AcceptanceMpt.scala:133-150`; `SnapshotLeaderLoop.scala:749-770,1227-1243,1339-1347`; `GlobalSnapshotContextFunctions.scala:380-403` | Every unknown/evicted snapshot hash silently reads the mutable finalized base. Unknown finalization can record a hash without applying its state; unknown replacement can undo the old base before recording an unapplied new hash. A validator can replay, slash, root, or finalize against the wrong bytes. A narrow unknown-finalize rejection was tested and rejected for rollout: normal finalization clears descendants and restart restores no authenticated base identity, so the next legitimate finalization can be unknown after the chain store/outbox already advance. | CONFIRMED, OPEN | Land one coherent transition: root-verified `(snapshotHash,ordinal,mptRoot)` base anchor, leased exact-parent acceptance session, descendant-preserving fold, crash-safe anchor persistence/recovery, preflighted finality orchestration, then typed `RecoveryRequired` before any external finality mutation. |
| SMT-03 | CRITICAL | IMPLEMENTATION | overlay ancestry/atomicity | `MptOverlay.scala:331-335,650-674,762-805,828-950,1181-1198` | `commit` records any parent/ordinal without proving that parent is available, while `mergedChain` silently stops at a missing ancestor. A child of evicted P can therefore be recreated and registered over unrelated base B, then finalized as if it extended P. On replacement, the undo entry is removed before a separate base transaction/callback/new fold, so failure can strand a reverted base with the old marker and no usable undo. | CONFIRMED, OPEN | Exact-parent session must prove complete ordinal-consistent ancestry to the verified base before replay/commit; make undo, replacement fold, anchor/marker update, descendant rebase, and recovery intent one crash-consistent transition. |
...
FIN-01..FIN-10, FIN-12, ECO-02..ECO-06, ECO-10..ECO-15, ECO-17/18,
SHARD-01..SHARD-10, SMT-01..SMT-03, SER-01/SER-02, and every open HIGH network
finding, is closed by adversarial tests.
