...
## FINDING-F3 — ChainSync inbound serve: dead rate-limit and no inbound hash caps → remote unauthenticated validator DoS
- **Severity:** High · **Category:** Security (DoS) · **Confidence:** High.
- **Location (VERIFIED @ HEAD 21933559c; partially retired 2026-07-13):** the original finding included an unbounded `serveFetchByRange` → JVM `(start to end).toList` amplification. That undeployed, unverifiable archival protocol and its daemon are now removed end-to-end. `RateLimitPerPeer = 10` is still declared but unused, and `MaxHashesPerRequest = 64` is still outbound-only; the inbound `serveFetchSnapshots` and `serveMetagraphBinaries` handlers remain bounded only by the frame limit.
- **Proposed direction:** wire `RateLimitPerPeer` into `handleIncoming` (per-peer token bucket) and enforce `MaxHashesPerRequest` on both remaining serve hops. The range-specific amplification is closed by removal, not by a cap.
