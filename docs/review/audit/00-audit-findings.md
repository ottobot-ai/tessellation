...
## FINDING-F3 — ChainSync inbound serve: dead rate-limit, no inbound caps, and the JVM amplifies (`(start to end).toList`) → remote unauthenticated validator DoS
- **Severity:** High · **Category:** Security (DoS) · **Confidence:** High.
- **Location (VERIFIED @ HEAD 21933559c):** `protocol.go:37` `RateLimitPerPeer = 10` is declared and **never referenced** (grep-verified); no throttle in `handleIncoming:94-133`. `MaxHashesPerRequest = 64` is enforced only outbound (`:248,:460`), not on `serveFetchSnapshots:135-173` / `serveFetchByRange:372-409` / `serveMetagraphBinaries:415-452` (bound only by the 16 MB frame → ~500k hashes/request). `serveFetchByRange` relays any `[start,end]`, and the JVM does **not** cap it: `ChainSyncServer.scala:204-216` runs `(start to end).toList.traverse_` — a strict List materialization of the whole span + per-ordinal disk read + JSON encode. A few-million span pins disk+CPU for the 2-minute timeout, repeatable in parallel; a ~2-billion span drives multi-GB allocation. `serveSnapshots` likewise uncapped (`:62-63`).
- **Proposed direction:** wire `RateLimitPerPeer` into `handleIncoming` (per-peer token bucket); enforce `MaxHashesPerRequest` + a `maxPullRange` on **both** serve hops (sidecar AND `ChainSyncServer`); iterate the range lazily (`fs2.Stream.range`, never `.toList`).
