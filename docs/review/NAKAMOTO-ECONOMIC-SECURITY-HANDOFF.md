...
**Repo state.** Branch `feature/committee-state-diff`; worktree rechecked 2026-07-16. All paths are under `modules/`.
...
canonical byte diff or signed per-input decision/disposition. The current replay
...
- **Emitter certificate gate.** `WatchtowerFraudProofEmitter.emit` first calls `verifyExecutionCertificate`; under-quorum or malformed candidates stop before replay/publication, while only a certificate-authenticated affirmative mismatch can be signed and published (`WatchtowerFraudProofEmitter.scala:90-109,114-139`). The daemon reaches this gate from the typed mismatch branch before storage (`NakamotoSyncDaemon.scala:2641-2653`) and from replay-valid best-tip adoption (`:2672-2686`).
- **Portable verdict semantics.** For each carried dispute, every GL0 validator authenticates the execution certificate before trusting signer IDs, then invokes the complete ordered checkpoint-batch replay once. The evidence MG selects only which attested root is compared; it never narrows execution. A reproduced-root mismatch upholds. For a structurally certificate-valid checkpoint, the only non-root execution outcome that upholds is explicit typed `ProvenInvalidTransition`; `Unavailable`, generic incomplete replay, missing/empty re-derived roots, and inability to reconstruct return `CannotRederive` and cannot slash. The code also has a missing-attested-root structural conviction branch, but the combined certificate gate makes it unreachable as described below (`InvalidStateProofValidator.scala:168-215`; production mapping `ShardCheckpointWiring.scala:239-314`).
- **HIGH OPEN — deterministic reject typing.** Production batch replay currently returns only `Reproduced` or `Unavailable`; it never emits `ProvenInvalidTransition`. Shared-fee oversubscription is serialized and the entire checkpoint is withheld, but the legacy processor exposes the rejected suffix only as generic `Incomplete`, so a colluding quorum that signs that unexecutable checkpoint is blocked from adoption yet escapes `InvalidStateProof` (`GlobalSnapshotStateChannelEventsProcessor.scala:145-220`; `ExecutionBasePinReExecutionSuite.scala:731-855`). The transition function must distinguish deterministic protocol rejection from missing/local dependency failure before this slashing path is complete.
