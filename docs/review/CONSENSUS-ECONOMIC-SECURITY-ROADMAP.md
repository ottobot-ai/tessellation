...
| MPT persistence ordering | `MptStore` starts one detached persist fiber per sync/commit and advances its in-memory ordinal immediately. Each fiber later reads mutable producer state, writes directly in place, and applies a cutoff keyed by its caller ordinal (`MptStore.scala:165-174,345-408`; `FileSystemMerklePatriciaProducer.scala:335-343`; `MptStateStorage.scala:45-84`). | A delayed persist for N can label N+1 bytes as N and delete an already-written N+1 image. E9-BRANCH needs immutable capture, serialized durable publication, and a receipt-backed watermark before runtime activation. |
