...
| Durable tower/SMT recovery | Current worktree replaces `Option`/`flatMap` omission with a total canonical durable-key grammar. Tower construction validates the complete key/value image once and materializes a private ordered index; hot reads and updates do not repeat tower-specific physical enumeration, decoding, or sorting. Generic MPT mutation validation still examines the current physical key set and remains O(n). Append/prune keep producer I/O cancelable in a savepoint transaction and mask the matching index publication only after success; post-mutation failure/cancellation restores producer, index, and persistence ordinal and releases the mutex (`MptTowerStore.scala:104-275`; `MptTowerStoreSuite.scala:260-450`). Historical replay validates the image and every `ordinal + k`, builds a fresh `VersionedSmt`, and atomically swaps the derived tree after success (`HistoricalCommitmentSmtStore.scala:116-225`). Double-replay retained-root parity and injected hashing failure/cancellation preservation are green (`HistoricalCommitmentSmtStoreSuite.scala:237-388`). | The source-confirmed silent-drop, repeated tower-specific enumeration/decode/sort, local transaction cancellation, and derived-tree partial-publication paths are fixed in focused worktree tests. This does not make generic MPT mutations sublinear, the durable KV crash-atomic, or the stores production-wired. The live catch-up records archival target A as settled before launching a detached walk bounded to 100 entries from prior P; when A>P+100, or an entry is missing/fails, the next tick starts at A and never retries the omitted interval (`SnapshotLeaderLoop.scala:362-403,1520-1572`). `TowerFinalizer` also recreates its de-dup watermark and metrics count at zero on construction (`TowerFinalizer.scala:73-75,152-179`). `REC-004` remains open for one post-success durable cursor, boot/disk wiring, restart and compaction, exact-hash shallow/deep density reorg, proof serving, catch-up, authenticated clean rebuild, and publication-blocking recovery. |
...
| P4 Operator registry/stake/KES/evidence | shared schemas, historical resolver, crypto/evidence validators, and owned integration | P1 identity primitives; O-11 before runtime eligibility | Rooted delayed population/key/stake binding, rotation, backing, false-slash, and exact-debit model. |
| P5 Lane/DA | signed envelope/registration/content commitment and bounded fetch | P0 lane answer, P1 | Currency and currency-with-data contract plus withholding/recovery tests. |
| P6 Durable FinalityGate/tower | finality/store, tower/SMT, owned GL0 integration, and FIN-14 lease kernel only after O-16 | P1/PF; economic release hooks after P2 oracle | Crash-consistent hash-bound Phase 2, density/deep recovery, consensus-reproduced `smtRoot`, portable tower proofs, and owner-ratified exact-consumer authority. |
...
2. Resolve the applicable open owner gates, including O-15 through O-17; runtime work must not
...
4. Freeze the complete root/write set, GL0 correction shape, and checkpoint
   preimage in P7 after O-17 and the dependent economic decisions.
