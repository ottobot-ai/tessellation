...
| Checkpoint execution-base identity | **Exact-state slice landed, authority remains open:** `ShardCheckpoint.executionBase` is now `GlobalSnapshotStateRef(ordinal,hash,parentHash,mptRoot)`, and all four fields are in the committee signing preimage (`GlobalSnapshotStateRef.scala:11-22`; `ShardCheckpoint.scala:61-76,94-104`). The pinned reader resolves that exact snapshot identity, requires its committed root, and recomputes the root from retained bytes before exposing state; a miss has no live/head fallback (`PinnedCurrencyInfoReader.scala:272-297,321-391`). The producer captures a canonical Scodec identity of the complete retained base, re-reads and byte-compares it after replay, and only then signs; receiver/watchtower replay resolves the same signed reference (`ShardCheckpointProducer.scala:208-257,420-454,681-718`; `ShardCheckpointWiring.scala:156-190,209-230,346-388`). The focused regression covers fail-closed same-ordinal wrong hash, parent, and root cases (`PinnedCurrencyInfoReaderSuite.scala:359-390`). | This closes ordinal-only sibling substitution for the retained execution prior; it does **not** authenticate Phase 2. `GlobalSnapshotStateRef` is explicitly a claim, while live Phase-2 authority still comes from the ordinal watermark/current-best-branch composition above. Shard eta, historical registry/roster, committee selection, and parameters are not resolved from this exact base; freshness, purpose domain, lease/CAS, and reorg invalidation remain open. Network/genesis/era/parameter binding, pre-roots, diffs, extracted intents, replay witnesses, and custom-lane commitments also remain absent. Therefore FIN-14, O-16, and protocol-test-plan P4.4 remain open. |
| Candidate replay authority | **Historical-lookup containment landed; exact replay session remains open:** the pinned-view fetch no longer prefers a same-ordinal `LastN` sibling, and historical `SpendAction` fetches ignore both `LastN` and the manager cache. Within one historical batch, each unique required ordinal invokes only the caller resolver once; all bounded outcomes are collected, any miss/error fails closed, and successful actions fold by ordinal (`CurrencySnapshotAcceptanceManager.scala:362-400,468-475`; `GlobalSnapshotOpsManager.scala:23-37,63-123`; `CurrencySnapshotProcessedSetSuite.scala:244-329`). Currency message validation still reads ambient combined global state, and an old field-emission fallback still observes `LastN` (`CurrencySnapshotAcceptanceManager.scala:300-324,589-599`). | The injected callback remains ordinal-only and separate producer/receiver paths can still construct it differently. This containment does **not** prove exact ancestry. The next slice must introduce one candidate-scoped artifact resolver plus an exact hash/root-bound historical state reader for every replay input; unavailable local history enters recovery/defer. |
