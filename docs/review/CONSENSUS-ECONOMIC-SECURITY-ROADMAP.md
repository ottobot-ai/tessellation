...
| Dormant GL0 BFT plumbing | At the baseline, `GlobalSnapshotConsensus` allocated an inherited unbounded `ConsensusEventLoop` queue and returned its six-family handler without running `loop.run` (`GlobalSnapshotConsensus.scala` at HEAD `:863-906,2382-2393`; `GlobalConsensusHandler.scala:14-35`). `Main` registered that handler but only constructed `gossipDaemon`; it never invoked either start method (`Main.scala` at HEAD `:259-325`). The handler therefore had no live dispatch path into the legacy queue. | This was a dormant architectural hazard, not a confirmed remote heap DoS: merely starting rumor consumption would have made the undrained legacy queue reachable. The worktree deletes the GL0 handler, constructs no event loop, and supplies no-queue fail-closed compatibility objects (`GlobalSnapshotConsensus.scala:74-115,2302-2310`). `GlobalLegacyBftIngressDisabledSuite.scala:26-82` rejects all six legacy families, proves the manager owns no queue, source-checks the GL0 cut, and preserves ML0 BFT; that suite is green (8/8), and the final combined receipt/BFT containment selection is green (35/35). The remaining generic `Consensus` storage/routes shell is compatibility cleanup, not authority. |
| GL0 sidecar-rumor consumption | At the baseline, `SidecarRumorBridge.receive` actively offered inbound rumors to the bounded shared queue (`SidecarRumorBridge.scala:60-105`), but `GossipDaemon.consumeRumors` is reachable only through `startAsInitialValidator`/`startAsRegularValidator` (`GossipDaemon.scala:28-32,56-70,89-116`) and `Main` called neither. Thus the configured generic/event rumor handlers were inert; the queue could fill and later inbound rumors would be dropped. | This is a confirmed wiring/liveness defect, but the exact affected workflow impact needs integration qualification because dedicated Nakamoto topics bypass this rumor path. After composing only generic and event handlers (`Main.scala:242-258`), the worktree starts the consume-only daemon after bootstrap and before `Ready` on both startup paths (`Main.scala:347-350,647-650`). The source regression is green, but this fixes only the consumer: runtime generic/event delivery, bounded draining, reconnect, and supervision/release remain unproved. |
| Early generic-rumor window | `Main` allocates `Services` and therefore `GlobalSnapshotConsensus` before entering rollback/genesis/restart bootstrap (`Main.scala:196-225,331-350,647-650`; `Services.scala:257-306`). `SidecarRumorBridge.receive` starts during that allocation (`GlobalSnapshotConsensus.scala:1121-1129`), while the new `GossipDaemon` consumer starts only after bootstrap. | **MEDIUM, pre-existing:** the bounded queue can fill and drop generic/event rumors during cold start. Do not call overall rumor startup safe. Move bridge activation after bootstrap, or specify a bounded pre-bootstrap quarantine with deterministic release/drop semantics and tests. |
| Early dedicated Nakamoto ingress | The same consensus allocation supervises `NakamotoSyncDaemon` before `Main` bootstrap (`GlobalSnapshotConsensus.scala:2192-2276`). Its dedicated snapshot worker immediately reads the chain/roster/economic services and invokes `handleSnapshot` (`NakamotoSyncDaemon.scala:845-910,1243-1271`). | **HIGH, pre-existing:** an early peer snapshot can race cold-start state establishment. Gate dedicated subscriptions/processing on one explicit bootstrap-ready capability, or quarantine exact bounded input without validation or mutation; test hostile early delivery, failure, restart, and release ordering. |
...
| Candidate replay authority | Currency replay reads the ambient combined global state for message inputs, prefers ordinal-only `LastN` artifacts for the pinned view, consumes ordinal-cached SpendActions, and retains an ordinal-keyed fallback cache (`CurrencySnapshotAcceptanceManager.scala:300-324,362,378-380,594-596`; `GlobalSnapshotOpsManager.scala:46-50,122-134`). | An exact-parent callback alone cannot make replay branch-deterministic. One candidate-scoped artifact resolver and exact hash/root-bound historical state reader must own every replay input; unavailable local history enters recovery/defer. |
...
| E4.8 | **LANDED CONTAINMENT; COMPATIBILITY CLEANUP OPEN:** delete dormant GL0 generic BFT event-loop construction, facilitator/round wiring, handler, and global BFT activation paths. The baseline queue was not remotely reachable because `Main` never started `GossipDaemon`; do not retain the retracted heap-DoS claim. It was still an activation hazard because starting the missing rumor consumer without first removing the six BFT handlers would feed an undrained unbounded queue. The worktree deletes `GlobalConsensusHandler`, constructs no `ConsensusEventLoop`, omits the consensus handler from GL0 rumor composition, and supplies no-queue fail-closed manager/handler compatibility objects (`Main.scala:242-258`; `GlobalSnapshotConsensus.scala:74-115,2302-2310`). The six-family/source regression is green (`GlobalLegacyBftIngressDisabledSuite.scala:26-82`; 8/8), the broader final combined receipt/BFT containment selection is 35/35, and ML0 explicitly retains its BFT engine. Remove the remaining dormant generic `Consensus` storage/routes shell and any GL0-only BFT config/API surface before calling E4.8 complete. |
| E4.8A | **CONSUMER LANDED; COLD-START ORDERING AND INTEGRATION OPEN:** restore GL0 sidecar-rumor consumption after E4.8 removes legacy BFT dispatch. The baseline constructed `gossipDaemon` but never started it (`Main.scala` at HEAD `:312-325`), so the bounded rumor queue had producers but no validator/handler consumer. The worktree starts `gossipDaemon.startAsInitialValidator` after economic-state/bootstrap setup and immediately before `Ready` on both startup paths (`Main.scala:347-350,647-650`); Nakamoto mode runs only supervised `consumeRumors`, not the legacy HTTP pull rounds (`GossipDaemon.scala:56-70,89-116`). The BFT source regression is green (`GlobalLegacyBftIngressDisabledSuite.scala:60-82`; suite 8/8). However, `SidecarRumorBridge.receive` already starts during earlier consensus construction (`Main.scala:196-225`; `GlobalSnapshotConsensus.scala:1121-1129`), so the bounded queue can fill/drop before the consumer exists. Fix that ordering and prove bounded queue drain, signature/collateral validation, generic/event handler delivery, hostile early input, reconnect, startup/release supervision, and unchanged dedicated Nakamoto-topic delivery. |
| E4.8B | **OPEN HIGH:** gate `NakamotoSyncDaemon` dedicated-topic processing on completed rollback/genesis/restart bootstrap. It is currently supervised during `Services`/consensus allocation (`Services.scala:257-306`; `GlobalSnapshotConsensus.scala:2192-2276`), before `Main` enters bootstrap. No snapshot, attestation, chain-sync result, metagraph binary, checkpoint, or fraud proof may validate or mutate cold state. Add one explicit readiness capability or bounded inert quarantine, plus early-hostile-input, bootstrap-failure, restart, reconnect, and release-order tests. |
