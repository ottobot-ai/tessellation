...
**Status:** Active implementation plan; `20/23` owner questions are dispositioned. O-01 through O-17, O-20 field-34 slash records, O-22 final-launch fraud-proof adjudication, and O-23 slash liability/bond tranches are ratified. O-18 transport/DA, O-19 migration policy, and O-21 optimistic decision evidence await owner responses. Remaining engineering/research/parameter/proof gates block their named dependent tasks.
...
| Bootstrap/recovery peer authority | **Narrow containment only:** `PeerSelection` carries the selected peer plus exact ordinal/hash; selection now queries only the unique largest ordinal cohort, rejects equal largest cohorts, and returns its unique largest hash cohort (`PeerSelect.scala:32-40,70-99,123-137`). GL0 requires the chosen peer's fresh metadata to repeat that tuple, and cached/fetched full genesis requires a valid signature plus the child-linked exact ordinal/hash (`Download.scala:47-75,576-599,643-674`). ML0 bootstrap now requires the signed currency snapshot to match the selected tuple and its supplied `CurrencySnapshotInfo` to reproduce the signed state proof (`modules/currency-l0/.../Download.scala:54-78,195-206`). | **HIGH remains:** the unique largest cohort is an unthresholded peer-reported plurality, not Phase-2/fork-choice evidence; an eclipse can control it. GL0 still deletes snapshots/checkpoints/MPT/caches/mempool before fetching, validating, objectively comparing, and transactionally staging the replacement chain (`modules/dag-l0/.../Download.scala:201-217,298-338`). The ML0 check proves cryptographic self-consistency, not that the signers are the registered historical ML0 facilitator quorum. Stage and authenticate the complete candidate chain/state first, verify objective GL0 selection or ML0 historical BFT membership/quorum as applicable, then atomically switch all sinks. The focused helper tests do not prove orchestration-level zero mutation. |
...
| E0.2 | Preserve the completed O-01..O-17, O-20, O-22, and O-23 owner dispositions in `CONSENSUS-OWNER-DECISIONS.md` and its answers companion. Keep O-18, O-19, and O-21 explicitly pending; close only the explicitly named engineering/research/schema/parameter/proof gates, and do not fill an unanswered owner choice with local policy. |
...
| E1.10A | Add `Hasher.forScodec`, returning a typed `ScodecV1Hasher` whose consensus operation requires one audited `ConsensusHashSchema[A]` binding the exact byte-aligned `ImmutableCodec[A]`, type-specific static domain, and frozen bounded size. It computes fixed SHA-256 over that canonically length-delimited static domain plus exact immutable bytes and exposes raw 32-byte digests for signatures. Network/genesis/era/parameter/exact-context bindings live in the versioned preimage `A`; callers cannot independently select a codec or ambient runtime domain. Globally inventory every production schema/domain, enforce unique `tessellation/<lowercase-segments>/vN` names, and forbid raw/projection schemas. It is not another `HashLogic` case and cannot fall back to Circe/Kryo. Add immutable signing/verification/hash-envelope APIs and make legacy `A: Encoder` overloads unreachable from consensus. |
...
A dark common-context candidate now distinguishes nonzero 32-byte network,
genesis, and consensus-parameter identities and composes them with
`ProtocolEraId.ScodecV1`. The bootstrap context is 33 bytes and omits
self-dependent genesis/parameter IDs; the ordinary context is 97 bytes.
Constructor-body validation and cloning protect direct JVM callers, the values
are not Java-serializable, and source tripwires reject direct production-source
references outside the reviewed files. This closes no manifest row or hash
schema. Parameter and genesis preimages must be derived in noncircular order,
while MPT internals and migration require separate row-specific contexts.
...
production. This does not alter live `GlobalStateKey.toHex` physical keys or close
the unenforced ROOT-008 key grammar.
...
goldens and strict negative tests pass 15/15. The inverse source guards pass 10/10,
...
production differential suite passes 26 tests. For token locks it binds
production signature/source ownership, exact native/currency scope, canonical
lane-specific genesis, parent, and complete signed payloads. Native and currency
iterative-batch cases compare exact accepted/awaiting/rejected decision sets,
aggregate balances and successor references after accepted prefixes,
claimed-replacement and lane-specific in-round fields, and the complete
address-keyed active-lock map. They cover child-before-parent retry,
forward/reverse order, insufficient-balance awaiting, mixed awaiting/permanent
rejection, malformed aggregate decisions, and stale/wrong-parent failure. The
wrong-owner, lane, genesis, fee, and replacement negatives preserve the exact
production rejection or explicit helper-boundary error. Raw reference inputs
are encapsulated by private adapter implementations and reach reference
execution only through internal projection methods.

The reference suite covers both orders of every transfer/
...
differential reproduces the live lower native GL0 and currency ML0 shape. Both
layers independently accept transfer 60 plus token lock 60 and allow-spend
creation 60 plus token lock 60 against prior balance 100; current sequential
application returns `AmountUnderflow` in all four cases. For the allow-spend
pair, native GL0 applies the allow-spend first and fails the token lock, while
ML0 applies the token lock first and fails the allow-spend. A private test-only
mixed capability over already source-validated bindings uses the candidate rank
transfer, allow-spend creation, token-lock creation and retains exactly one
funded prefix without partial state. That rank is not activated or normative,
so canonical heterogeneous ordering remains an activation blocker
...
GL0 acceptance path, full heterogeneous production-batch orchestration, E2.8
cross-platform property corpus, canonical Scodec/hash/signature bytes, MPT
writes, complete-root calculation, or runtime activation. Production batch
insufficiency is `Awaiting` while the reference row rejects, and live GL0 accepts
an allow-spend outside the target reference epoch window. Legacy payload
signatures do not bind domain/lane; production exposes no independent replay-ID
or write-order evidence; batch allow-spend APIs do not expose active-record
deltas; single-result observers consume caller-supplied results and prove no
invocation provenance. Token-lock nonzero fees and replacement remain
helper-fail-closed because their target semantics are not frozen. The
differential explicitly records that live snapshot acceptance accepts a lock
rejected by the stricter reference contextual minimum-duration rule. Token-lock
...
uses the owner-ratified fixed-hash `BondId` plus complete `E-2` tranche
liability, full-only V1 with amount-changing bonded replacements and
operator/family retargets rejected, bounded hold/release horizons,
slash-before-release ordering, consumed-bond
tombstones, reward conservation, and portable historical Phase-2 revalidation.
The focused `InvalidStateProofSlashPrincipalRedSuite` compiles and
...
| E12.4 | Peer selection/cooldown/scoring affects transport only, never consensus roots, committee derivation, evidence truth, or finality. The current selected-tuple/fresh-metadata and signed-genesis/currency-context checks are containment only. Recovery stages and authenticates the complete candidate chain/state, performs objective GL0 comparison or verifies historical ML0 BFT membership/quorum as applicable, and atomically switches every sink before deleting the prior generation. Eclipse/plurality claims and helper-level tests cannot authorize cleanup. |
...
|---|---|---|
| W0 | E0 architecture/decision register, RED exploit corpus, finding ledger, test harness | O-01..O-17, O-20, O-22, and O-23 remain recorded as owner-ratified; O-18, O-19, and O-21 remain visibly pending; dependent engineering/research/schema/parameter gates and the capability matrix are explicit and assigned. |
