...
| Physical MPT key grammar | Multiple rooted partitions reconstruct logical maps from `entries.values`, embedded fields, or `headOption` without proving that each actual physical key equals the canonical key derived from the decoded value. This affects Mg fields, consumed-allow-spend nullifiers, stake/collateral, token locks, slash/cooldown state, price state, and node parameters (MPT-01 through MPT-06; representative Mg path `GlobalStateConverter.scala:1617-1629`). Separately, case-sensitive `Hex` keys normalize case-insensitively into trie paths; reproduced aliases hang full construction and make incremental roots disagree with retained bytes (MPT-07; `Hex.scala:26-28`; `ParallelMerklePatriciaProducer.scala:112-141,164-203,227-248`; `IncrementalTrieOps.scala:17-23,178-189,232-250`). Shared strict transport now retains immutable value bytes, null/empty/malformed entries, deterministic physical-key order, and upper/lower case aliases for the same trie nibble prefix (`StrictMptRead.scala:18-30,55-104`; `MerklePatriciaProducer.scala:43-66,102-115`; `InMemoryMerklePatriciaProducer.scala:40-56`; `FileSystemMerklePatriciaProducer.scala:57-73`; `MptStore.scala:259-273,400-403`). MultiBranch scans merge only the matching raw prefix under the overlay mutex before decoding (`MptOverlay.scala:828-845`); ownership/case/backend regressions are in `StrictMptEnumerationSuite.scala:37-176`. | A valid root can still authenticate a semantically misplaced, noncanonical, or duplicate logical record because no affected consumer yet runs a whole-image physical-key preflight, derives/checks canonical key/scope, or rejects duplicate logical identities. Raw recovery can also clear live state before an aliased candidate hangs in root construction. The new transport does not prove exact parent, branch, root, snapshot, or Phase 2. Complete `ROOT-008` and `ROOT-011` before parser migration or raw-state installation. |
