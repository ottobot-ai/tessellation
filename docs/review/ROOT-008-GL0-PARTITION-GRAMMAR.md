...
Within fields 8 and 13-16, the current focused transition now captures active
locks plus active/pending stake and collateral together and requires every
encumbrance to bind one unique live native same-source indefinite covering lock
(`StakeBackingValidator.scala:26-30,296-396,442-472`). It validates terminal
replacement chains and the complete changed post-state, and collateral maturity
removes its pending record and lock with one exact credit
(`StakeBackingValidator.scala:510-568`;
`GlobalSnapshotAcceptanceManager.scala:1968-2017,2727-2762`;
`NodeCollateralWithdrawalExpiryRootParitySuite.scala:60-257`). This is focused
ECO-02 lifecycle closure, not ROOT-008 or activation closure. Unchanged raw
installed images, lifecycle/recovery, resource bounds, and hash-era behavior are
open; ECO-06 can evade principal slash through a pending withdrawal. ECO-30's
historical cross-event snapshot abort is fixed by opposite-pending validator
checks and accepted-only collateral transition arbitration, while the final gate
remains defense. Unknown or evicted branches also still alias to base.
...
| 13, 14 | GSI/accumulator typed writer plus shared structure/backing gate | strict reader `StakeCollateralMptReader.scala:19-280`; writer gate `GlobalStateConverter.scala:66-136,717-810,984-989,1150-1172`; whole backing validator `StakeBackingValidator.scala:26-30,296-396,442-472,510-568`; opposite-pending check `UpdateDelegatedStakeValidator.scala:216-237`; final fold `GlobalSnapshotAcceptanceManager.scala:2727-2762`; exact-parent reward factory/use `Services.scala:132-160`; `GlobalSnapshotConsensusFunctions.scala:249-260,628-633,910-933` | **FOCUSED MPT-03/ECO-02 PARTIAL:** canonical placement/identity, exact-parent validation, atomic parent capture, unique live indefinite covering backing, safe bound replacement, and complete changed-state validation are enforced. ECO-30 cross-family pending-lock reuse rejects before materialization. A dark exact-parent N-2 reader exists but is not production-wired. Pending withdrawal still evades the active-only slash fold (ECO-06); whole-image/raw ingress, recovery, bounds, hash era, and broad oracle remain open. |
| 15, 16 | GSI/accumulator typed writer plus shared structure/backing gate | strict reader/writer as fields 13/14; opposite-pending check `UpdateNodeCollateralValidator.scala:205-227`; accepted-only transition and reused-lock-owner arbitration `UpdateNodeCollateralAcceptanceManager.scala:74-149,168-200`; maturity fold `TokenLockStateManager.scala:842-930`; `GlobalSnapshotAcceptanceManager.scala:1968-2017`; regression `NodeCollateralWithdrawalExpiryRootParitySuite.scala:60-257` | Same focused field grammar/backing contract. Collateral maturity removes the pending record and exact lock, credits once, removes its expiry bucket, and cannot credit again. ECO-30's cross-family reuse, direct successor/predecessor, and non-latest reused-lock-owner withdrawal aborts now reject at event arbitration. Field-16 expiry still consumes local `WithdrawalTimeLimit` (ECO-IDX-03), and pending collateral is absent from slash principal selection (ECO-06). |
...
| 20 | GSAM period-boundary accumulator plus typed writer (`GlobalSnapshotAcceptanceManager.scala:1409-1458,1502-1551,2638-2687,2711-2739,2770-2808`) | live ambient reader `HistoricalStakeReader.scala:52-58`; dark exact reader `HistoricalStakeReader.scala:18-40,60-120,152-161`; exact capture `MptOverlay.scala:1052-1072` | CONS-09's post-transition boundary write is fixed. The dark reader now proves exact captured base/parent roots and derives a read key under the historical write ordinal, but no eligibility consumer uses it. The current value still lacks `EtaPeriod`, so whole-image authentication and removal/rekey across a hash era remain impossible. CONS-04/CONS-08 and activation are NO-GO. |
...
|---|---|---|
| immutable genesis constructor | 2, 8, 11, 13, 15, 24 | exact public initial balances plus owner-signed stake/collateral event and signed native indefinite exact-amount backing-lock bundles, and validated rooted operator keys before the first live root; no runtime update. No private/secret field is public input. Historical ECO-29 owner keys are forbidden. |
