...
| **Protocol-era identity/cutover** | **IDENTITY/CLEANUP LANDED; RUNTIME OPEN** | The unused configurable `EraCodecRegistry` and invalid legacy bridges were deleted. `ProtocolEraId.ScodecV1` plus strict tag/vector tests are dark only; live `HashSelect` remains ad hoc in `TessellationIOApp.scala`, and the atomic Scodec runtime cutover is still open. `ERA-REGISTRY-DESIGN.md` is a superseded historical proposal. | (see 21) | **E1 atomic cutover; no piecemeal wiring** |
