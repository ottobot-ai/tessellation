...
| **Queue-boundedness audit** | **PARTIALLY-BUILT** | `QUEUE-BOUNDEDNESS-AUDIT.md` (trigger: OOM'd 4/5 gl0). `dag-l0/.../nakamoto/ChainSyncRequestQueue.scala` bounded; **open Tier-1/2 findings**: GossipStream bridge, consensus output queues, `NakamotoSyncDaemon` pendingParentRef, ShardBinaryBuffer accumulate on stalled peers. | Y — Cap *pending/unresolved* state globally, or TTL-per-use-case? (unbounded pending = liveness/DoS surface) | Reliability hardening; independent |
