...
| Owner decisions | Ratified optimistic/depth algebra, execution threshold, positive watchtower release, root fields, ML0 reorg rule, one-checkpoint batching, lane scope, and explicit remaining numeric/resource gates. O-18 and O-21 plus the post-register S2-ORDER-01..03, P7-CPREF-01..06, O13-A1 through O13-F including A3's exact schema/disposition, TL-EXP-1..4, and reward-algorithm choices remain owner stop-lines. O-19 is deferred with launch-profile-specific P12 and does not block greenfield runtime work. |
...
|---|---|---|---|
| P0 Architecture/decision/RED ledger | docs, drift guards, finding ledger | immediately | Ratified owner answers or an explicit non-launch/deferred disposition, forbidden-pattern guard, and every CRITICAL/HIGH assigned. |
...
| P5 Lane/DA | envelope/registration/content commitment and bounded fetch | O-09 ratified lane scope, P1; semantic schema/RED work may remain dark while O-18 is open, but live DA/intake wiring requires O-18 | Currency, currency-with-data, and retained DA-only opaque-lane contracts plus withholding/recovery and decoder-promotion tests. |
| P6 Durable FinalityGate/tower | node-shared finality store, tower/SMT, owned GL0 integration, and a dark FIN-14 lease kernel under O-16's ratified direction | P1/PF; economic release hooks after P2 oracle; phase/lease schemas may remain dark, while runtime tower/eligibility additionally requires the complete-root program, executable O-11 roster/history, and O-16 gates | Crash-consistent operational phase, load-bearing tower/SMT, arbitrary-depth density recovery, exact event outbox, and owner-ratified exact-consumer authority. |
| P7 Diff/root/checkpoint | P7.0 first freezes the authority-free shared exact-context/checkpoint-reference schema; the global state-reference canary is frozen, while the checkpoint-parent lineage value and `ShardOrdinal` rules await `P7-CPREF-01..06`; P4.4 then wires historical verification; remaining P7 owns checkpoint diff/root models/codecs, complete-root ownership, field-32 replay witness, and strict physical entries | P7.0: P1 plus P6 exact-ref value/evidence shape and owner disposition of `P7-CPREF-01..06`; remaining P7: P1/P2/P5/P6, P4.4 verified context, and root prerequisites | ROOT-007 complete coverage, ROOT-008 strict key grammar, ROOT-010 exact replay witness/removal order, and exact apply-diff vectors. |
| P8 Shard execution/watchtower | sharding runtime, positive coverage, challenge quarantine, challenge-triggered exceptional replay verdicts, and the pure certificate/base/CAS/diff/root adoption primitive; no live GSAM cutover and no universal GL0 replay for unchallenged ordinary checkpoints | P7 | Replay-before-sign, distinct positive assigned-watchtower coverage, deterministic match/mismatch/unavailable challenge verdicts, pure zero-recreation adoption vectors, and staircase/anchor/challenge tests. |
...
| P9 Cross-metagraph settlement | global pure kernel, the sole E9-owned live GSAM cutover through P8's adoption primitive, and consumed-nullifier parser migration | P2/P6/P8, E6 one-outstanding lifecycle, E7 authenticated custody, and P7 strict-entry interface | Allow-spend exact-once across shards, physical-nullifier binding, universal global-kernel execution, reorg, restart, and acknowledgement. |
...
| P12 Snapshot-genesis migration | isolated deferred offline tooling for a source-network hard-fork launch profile; not a greenfield-runtime dependency | Explicit O-19 owner disposition, then P1/P2/target state schema | Reproducible upstream-snapshot to ScodecV1 genesis and conservation report. |
...
|---|---|---|---|
| P4.0 Atomic authority inventory (`PARTIAL`: atomic API, raw/semantic tripwires, frozen K7a resolver vectors, effect-obligation inventory, and bounded K7b-1/K7b-2 producer/attester evidence landed; remaining adapters and historical qualification open) | One paired KES+VRF API; transport keys are evidence only; split mutable registries and direct-key consumers denied by inventory. Read-only KES/VRF projections may derive only from the atomic pair. The raw tripwire scans split factories, primitive references, and direct constructors. `OPERATOR-CONSENSUS-KEY-SEMANTIC-MANIFEST.tsv` separately inventories reviewed lexical spellings and file/counts for higher-level draw, eligibility, duty, signing, proof, tower, and evidence calls, including blocked absent roles, and pins uncommented/unquoted negative-vector test-shaped declaration names. Its checked zero-effect obligation vocabulary covers draw, eta, possession, replay, both signature domains, aggregation/tracking, store/adopt/publish, and slash. Hard-coded reviewed evidence metadata qualifies `KSEM-EXEC-003..011`. K7b-2 directly counts producer eta lookup, duty ordering, possession proof, per-MG derivation-hook invocation, KES, and publish; scheduled-duty selection and Ed signing are source-dominated. K7b-1 directly observes attester eta/proof/KES/tracker/publish with Ed signing source-dominated. Both hooks are stubs: producer `Replay` evidence means derivation-hook invocation only, and neither cut proves CL1 replay semantics or economic correctness. Separately, the checkpoint boundary now withholds a per-MG root/adoption result unless the legacy processor returns the exact complete ordered signed-envelope window; this closes accepted-prefix/reordering/substitution authorization, not full CL1 replay correctness or decisions/diff/intents parity. A dark single-slot opaque-byte CAS/custody store now covers only local durability mechanics; it has no checkpoint codec, publish/release operation, or restart-republish authority. Exact hash/root-bound execution base, the production pre-publication outbox/`SHARD-C-008`, and historical committee/membership/runtime activation remain open. Other obligations remain unproved. Same-file line motion/substitution is not detected, and neither inventory nor anchor mapping is an AST, symbol-resolution, call-graph, or branch-historical proof. | P1 identity bytes. | `KEYREG-001`, `KEYREG-005`, `KEYREG-006`, `KEYREG-010`, `KEYREG-011`; checked semantic inventory has zero unapproved production key/sortition call sites and every allowlisted path passes branch-historical vectors. |
...
| P4.4 Exact Phase-2 artifact-reference verification | Currency binaries and checkpoint bases/anchors use the P7.0 exact `(ordinal,hash,parentHash,mptRoot)` shapes; P4.4 makes their Phase-2, registry, eta, roster, and stake resolution branch-historical and load-bearing. | P4.3; P6 phase proof; P7.0 authority-free shared exact-context/checkpoint-reference schema, not full P7 integration. | `KEYREG-006`, `KEYREG-007`, `KEYREG-010`, `SHARD-E-003A`, `SHARD-C-001B`, `FOLLOW-001`; same ordinal/wrong hash or root fails producer, signer, acceptance, reorg, and adjudication paths. |
...
| P4.6 Fixture migration and release guard (`PARTIAL`: fixture migration, tripwires, generated frozen-view resolver matrix, and K7b-1/K7b-2 producer/attester qualification landed; remaining adapters and historical qualification open) | Every consensus fixture commits the complete genesis pair/population or registers the pair in the exact canonical N-2 view before use; arbitrary keys remain only in low-level crypto implementation tests. `CommitteeSortitionSuite`, `CommitteeShardSortitionSuite`, and `EligibilityCheckerSuite` use loader-validated period-zero pairs, including registered wrong-key controls and repeated canonical-input statistical draws. K7a generates authority A plus a loader-registered B absent from A's frozen view and rejects absent/substituted/unrooted-runtime-shaped/malformed identities through the real frozen resolver. The runtime-shaped record has a dummy proof and is not loader- or cryptographically validated. K7b-1 qualifies the frozen execution attester through its private-capability boundary. K7b-2 applies all five missing/A-B substitution/unrooted/malformed cases to shard-producer fresh mint. Held re-publication covers authority disappearance, loader-validated B's VRF bytes substituted into the live local array, and the unrooted runtime-shaped A record; immutable long-term and array-length constructor inputs remain fresh-only without a production seam. Rejections directly observe zero eta lookup, duty-order draw, possession proof, per-MG derivation-hook invocation, KES signing, execution-base reads, and publish. Positive held A re-publishes exact bytes without another eta/duty/proof/hook/base/KES effect; scheduled-duty selection and Ed signing are source-dominated. The producer `Replay` label means only derivation-hook invocation, not actual CL1 replay. Neither cut proves economic correctness, direct outer signing, store/adopt/slash, historical committee/membership, or runtime activation. Separately, the checkpoint replay boundary now rejects accepted-prefix/reordering/substitution authorization by exposing results only for exact complete ordered signed-envelope windows; it does not prove full CL1 replay correctness or decisions/diff/intents parity. An exact hash/root-bound Phase-2 execution base and durable pre-publication checkpoint outbox remain open. The cuts do not replace the generated historical branch/era cross-consumer matrix or qualify blocked watchtower/optimistic consumers, portable nonempty towers, or positive historical slashing/runtime paths. | P4.5 APIs frozen; may migrate independent fixture families earlier. | `KEYREG-006`, `KEYREG-010`, `KEYREG-011`; generated-unregistered key fails before every draw/proof/sign/attest/store/adopt/slash side effect and the inventory artifact reports zero exceptions without an explicit primitive rationale. |

For K7b-2, outer-envelope signing is source-order dominated by the directly observed
zero KES call. It has no manifest effect label or direct counter, so this is not a
direct outer-signature evidence claim.
...
undo/refold -> P4.3 exact-parent N-2 population/key and N-1 eta capability ->
P7.0 shared authority-free exact-context/checkpoint-reference schema -> P4.4
exact Phase-2 historical verification -> remaining P7 integration and P4.5
consumer/evidence migration -> P4.6 fixtures and qualification. The paired-key
portion of P4.1 may progress independently, but no
...
| S5 Watchtower release and challenge verdict | Deterministic noncommittee assignments and capability-backed positive replay coverage are the ordinary gate. An authenticated assigned mismatch quarantines and triggers bounded exceptional universal GL0 replay of exact data, returning match, mismatch, or unavailable; unchallenged ordinary checkpoints are not universally replayed. This stage has no slash/bounty/rollback/delivery authority. | S4 checkpoint plus authenticated S3 custody. | No checkpoint is GL0-eligible without minimum positive coverage; an authenticated mismatch quarantines regardless of positive count; match can release quarantine, unavailable defers; false positive/negative, unavailable-data, partition, flood, and restart suites pass. |
| S6 Ordinary GL0 adoption | P8's pure execution/coverage certificate, diff, base, CAS, namespace, and root primitive followed by E9's sole live GSAM cutover. | S4/S5 plus E6 one-outstanding lifecycle and E7 custody. | Valid certificates and diff adopt with zero framework recreation; invalid assignment, duplicate/nonmember coverage, bad domain/signature/threshold, and every malformed/stale/cross-namespace diff leave state unchanged. |
...
| S8A Fraud/slash effect integration | Portable InvalidStateProof adjudication reuses S5's challenge-triggered verdict contract, then atomically applies slash/bounty delivery and rollback/reorg effects. Assertions and missing data never slash. | S6-S8 plus O-20/O-22/O-23 schemas and economics. | False-positive/false-negative, actual-debit funding, tombstone, slash-before-release, restart, and density-reorg suites converge without changing an already-applied verdict by local history/config availability. |
...
| S10 Permissionless qualification | Multi-node transport, eclipse/partition/restart/long-offline, resource and performance envelope, source re-audit. | S0-S9 including S8A. | Independent agent reproduces every applicable suite on the release commit with no open unowned CRITICAL/HIGH finding. `PARAM-001`, zero consensus effect from receiver-local config/wall clock, and `SHARD-C-005` are unconditional. |
...
| SER-005 | `Hasher.forScodec` returns a typed `ScodecV1Hasher`; digest requires one audited `ConsensusHashSchema[A]` binding the exact byte-aligned `ImmutableCodec[A]`, type-specific static domain, and frozen bounded size, and is fixed SHA-256 over the canonically length-delimited domain plus exact ScodecV1 bytes. Network/genesis/era/parameter/exact context are fields of the versioned preimage `A`, not ambient inputs. Immutable signing verifies the raw 32-byte digest. Every production schema is allowlisted with a globally unique `tessellation/<lowercase-segments>/vN` domain; `Json`, `Array[Byte]`, `ByteVector`, `String`, and generic pass-through types have no schema. Every active hashing/signing call uses that API under one canonical era service. It is not a `JsonHash`/`KryoHash` selector case. Repository/runtime guards reject JSON/Kryo `Hasher`, Circe-derived signing preimages, UTF-8 hex-digest signing, caller-selected schemas/domains, and local ordinal switches on consensus paths. |
...
| PARAM-001 | Independent implementations derive exact k1/k2/R/eta periods, committee thresholds, duty windows, challenge horizons, and resource limits from one parameter object. No `Double`, local env, overflow, or rounding-mode difference changes a result. The current 102-row source-authority inventory and guard account for all typed Nakamoto HOCON overrides, conservatively trip on Scala/Go source files combining raw environment APIs with Nakamoto keys, freeze the reviewed raw token/version/jar identity overrides and local allowance-list join gate, and include the reviewed node-local consensus clocks; that lexical tripwire is not a whole-program data-flow or completeness proof and closes no runtime parameter-authority requirement. |
...
| ECON-REWARD-003 | Preserve the reachable validator-to-two-window-price-state RED vector: Testnet accepts the exact price at epochs `1001105/1001189/1001273`, makes it current, and the live Math versus StrictMath paths mint `30031351592` versus `30031351591` at epoch `1001274`. Freeze the intended real-valued curve, exact rooted input bounds, zero-price rule, precision schedule, proved outward error interval, and hard nonconvergence/resource failure. An independent rational/fixed-point oracle and production implementation must choose the same HALF_UP integer for every bounded input on every supported JDK/CPU, including boundary, overflow, underflow, zero-impact, pre-transition, maximum, and exhaustion vectors. The deterministic target replaces fork-only binary64 execution at ordinal zero; no active-chain compatibility decoder/path remains, and any upstream-v4 source verification is isolated migration tooling. |
| ECON-BACKING-001 | Every active or pending delegated-stake/collateral record binds exactly one live native same-source indefinite lock covering its effective amount. Duplicate/cross-family binding, missing/finite/short/wrong-source lock, same-ordinal replacement chain, withdrawal, maturity, reward mutation, slash, restart, and reorg either preserve that join or apply one explicit terminal transition. **Focused ordinary-transition GREEN only:** `StakeBackingValidatorSuite`, `TokenLockBlockAcceptanceLogicSuite`, coordinator/token-lock manager suites, and `NodeCollateralWithdrawalExpiryRootParitySuite` cover the current join/replacement/maturity paths. Raw installed images, recovery, bounds, hash-era behavior, and full oracle remain open. |
| ECON-BACKING-ISO-001 | Opposite-family pending-lock reuse, direct collateral successor plus predecessor withdrawal, and a create reusing a non-latest collateral lock plus withdrawal of that old owner record reject as attributable events before materialization; rejected inputs reserve no context and unrelated inputs continue. The final whole-state gate remains defense. **Focused worktree GREEN:** `UpdateDelegatedStakeValidatorSuite.scala:375-401`, `UpdateNodeCollateralValidatorSuite.scala:315-348`, and `UpdateNodeCollateralAcceptanceManagerSuite.scala:155-276`. |
| ECON-COLLATERAL-RELEASE-001 | Maturity removes the exact pending collateral record, backing lock, and expiry-index member, credits exact principal once, and the next ordinal cannot credit again. Producer accumulator replay, rebuilt GSI/MPT bytes, and signed root agree. **Focused worktree GREEN:** `NodeCollateralWithdrawalExpiryRootParitySuite.scala:60-257`; restart/density-reorg/slash interaction remains required. |
| ECON-SLASH-001 | InvalidStateProof V1 is full-only over one unique exact-amount lock per fixed-algorithm offense-time `BondId` tranche unless a residual-lock protocol is separately designed. The complete liable set is all operator security tranches in the exact rooted `E-2` view; draw probability remains uniform. Exact-amount replacement and pending withdrawal preserve the tranche; every amount-changing bonded replacement or operator/family retarget rejects, and a changed position requires a separate exact lock/new `BondId` after prior liability permits release. Debit exact active or pending backing atomically with lifecycle maps, release index, consumed-bond tombstone, per-signer field-34 identity, bounty, cooldown, accumulator, and evidence. A second distinct offense by an already consumed bond records culpability with zero debit only through the tombstone. After density replacement, portable old-branch evidence may record zero-debit culpability for a genuinely absent bond; divergent same-ID or unexplained missing backing rejects/enters recovery. Slash wins release collisions. Principal/reward conservation, portable historical Phase-2 revalidation, restart, and density reorg reproduce one root without resetting deadlines. **CRITICAL RED/OPEN:** `InvalidStateProofSlashPrincipalRedSuite` compiles and fails 5/5 intended cases; SHA-256 `96f568291a4eee8e636debd814b8d8e4a3d5eebc6c3c2e06a5a752f4b3c04ca7`. |
| GENESIS-ECON-001 | Fresh greenfield public genesis contains only owner-signed stake/collateral events plus exact same-owner signed native indefinite backing locks, zero rewards, exact ordinal/parent/amount/ref, and explicit spendable plus backing allocation. Loader, first-live MPT rebuild, and signed root reproduce the same state; malformed bundles fail before publication/startup. |
| GENESIS-SECRET-001 | Public generated/tracked artifacts contain no private/secret field or EC PKCS8 bytes. Secret directories/files are non-symlink 0700/0600, secrets publish before the public L0 anchor and are zeroed on success/failure/cancellation. Every owner address whose private key appeared in git history is denied from deployment/import permanently; a history rewrite is not revocation. |
...
| SHARD-E-001A | The legacy currency processor may retain its non-checkpoint accepted-prefix behavior, but checkpoint root/sign/adoption callers receive a per-MG result only when the returned oldest-first hashes equal every exact ordered `Signed[StateChannelSnapshotBinary]` input. Valid N followed by undecodable/failed N+1, same-count substitution, reordering, omission, and unexpected MG output cannot authorize a prefix root or adoption. This input-consumption gate does not by itself prove CL1 transition correctness or future diff/intents parity. |
...
| SHARD-E-006 | **Current-failure characterization GREEN; fixed-path matrix OPEN.** `ExecutionBasePinReExecutionSuite.scala:573-643` reproduces ECO-F32 with one node reading a locally staged nonempty field-32 base and another reading a peer-backfilled map that verifies the same signed GL0 root after field-32 stripping. The same signed binary replays only from the local image, and `None` versus `Some(empty)` produce distinct proof shapes. The fixed paths must consume the same exact optional full-view witness and explicit ML0 operator population and reproduce byte-identically. Missing/wrong preimage, wrong population, hash mismatch, and `None`/`Some(empty)` substitution defer before signing and cannot create slash evidence. |
...
| SHARD-C-012A | **Partial producer/attester containment GREEN; recovery OPEN.** Missing local GL0 lineage generation defers before checkpoint replay/sign/publish. Replacement observed during producer or attester replay withholds every signing capability; on the attester path it also withholds the replay-rejection result. Replacement after attester capability acquisition but before emit suppresses KES/tracker/publish, while replacement before a producer held retry clears the volatile memo and emits no bytes; forced ABA cannot revive it and descendant extension preserves exact-byte retry. If the orphan checkpoint is already the local shard tip, the producer fail-stops instead of clearing/reminting it. The tests do not atomically bind the separately read lineage and execution base, authenticate exact Phase 2, revoke already-gossiped bytes, cover the post-return daemon store/carried-signature-record race or stable-rejection-to-separate-watchtower-replay race, supply authenticated old/new/MRCA reorg evidence, replace the shard tip/frontiers/anchor, close the final emitter check-to-act race, or provide restart durability, so they close neither `SHARD-C-012`, `SHARD-C-008`, `FOLLOW-008N`, nor FIN-14. |
| SHARD-C-012B | **Partial watchtower containment GREEN; adjudication/recovery OPEN.** Missing local GL0 lineage suppresses watchtower replay and proof emission. Replacement across replay suppresses the mismatch; later replacement suppresses Ed25519 or each publish/retry attempt. Inbound proof lookup plus validator replay must observe one generation, pool offer rechecks it, and proposal peek atomically prunes mismatched/absent tags; descendant extension preserves eligibility and replacement pruning prevents forced ABA revival. The local tag is absent from every schema, preimage, root, and verdict. RED completion still injects replacement in the final effect-read races, starts with an already-orphaned checkpoint in the untagged shard store, exercises already-gossiped bytes, restarts, and proves candidate-parent exact-history plus branch-aware `commitIfCurrent`. It must also prove that history/config asymmetry cannot make one GL0 uphold while another maps `CannotRederive` to no-slash. This row closes neither `SHARD-C-012`, `WT-006`, FIN-14, `O-16`, nor `O-20`. |
...
| Test ID | Required scenario and assertion |
|---|---|
| WT-000 | **Temporary source-level development interlock, not a protocol era:** until the direct final path replaces it, the provisional local pool cannot reach proposals and nonempty provisional evidence rejects before GSAM, MPT, overlay, storage, serving, finality, or committee selection. Different local watchtower/slash/bounty/cooldown HOCON cannot change an artifact or root. Final integration replaces the provisional field once with bounded `InvalidStateProofEvidenceV1` in the sole ordinal-zero ScodecV1 launch schema; no mandatory-empty/removed target era, later activation, JSON authority, or compatibility decoder exists. **Focused RED:** the isolated `O22FinalLaunchWireRedSuite` specifies the final active launch path rather than pre-activation absence. It currently reports four failed target cases and one passing known-byte rejection: JSON digest cutover, bounded canonical V1 evidence, atomic five-result field-34 integration, and local-HOCON independence remain RED; the provisional/trailing Scodec fixture rejects. Exact hash and promotion conditions: `CONSENSUS-RED-WITNESS-CATALOG.md`. |
| WT-000A | Final launch producer/verifier, storage, combined-checkpoint, ML0 envelope, HTTP, GossipSub, ChainSync, restart, and catch-up paths retain and authenticate the exact final bounded ScodecV1 evidence bytes. JSON is API/debug projection only and no decode/re-encode path can create a signature preimage. The provisional evidence shape and 27-field Scodec bytes, unknown tags, trailing bytes, malformed refinements, duplicate identities, unsorted collections, and over-limit values reject. Legitimate upstream-v4 Kryo/Brotli data is readable only through the isolated offline importer. |
...
| WT-006A | A colluding execution quorum signs a genuinely wrong checkpoint on branch A. After a Phase-2 density replacement to B, every GL0 resolves the exact historical A base and reaches the same objective verdict; none may translate local pruning/absence into no-slash while another upholds. The guilty signer set, debit, bounty, cooldown, and rooted result are identical under asymmetric caches, retained history, restart, and local environment. Until exact proposal-parent history and rooted slash parameters satisfy this test, watchtower slashing is activation-blocked even though local lineage containment is GREEN. |
| WT-007 | Forged, duplicate, conflicting, unavailable-input, and flood challenges cannot false-slash or force unbounded all-GL0 replay of a challenged sharded-CL1 checkpoint. Assignment, challenger bond, rate/resource limits, and deterministic exceptional replay by every GL0 validator of that checkpoint's exact retained base and ordered framework inputs survive restart/partition. Native GL1 and global-kernel execution remain universal independently of a challenge. |
...
| WT-010 | Every rooted invalid-state-proof record proves its actual physical key equals the fixed-Scodec/SHA-256 signer/shard/disputed-checkpoint identity decoded from the value. Its committee effect is the half-open rooted artifact-period interval `[excludedFromPeriod, eligibleAgainAtPeriod)`; period-boundary, wrong-unit, and local-parameter substitutions reject. Field 34 alone cannot change bonds, balances, bounty, burns, tombstones, or supply; the separate O-23 economic delta is atomic with it. Wrong-key placement, duplicate logical identity under two keys, signer/scope substitution, malformed value, ambient-hash-era drift, and map-order collision reject before evidence or eligibility effects. Live replay, restart, and density reorg cannot redirect, omit, duplicate, or escape a record. |
| WT-010B | Event-triggered snapshots advance ordinal without advancing `EpochProgress`; time-triggered snapshots advance both according to their actual rules. For every schedule, a slash exclusion begins exactly at rooted `excludedFromPeriod`, remains active through the last included artifact period, and ends exactly at `eligibleAgainAtPeriod`. The old mixed condition `eventOrdinal <= ordinalAnchor < EpochProgress+cooldown` is reproduced as RED and can never return under another field name. **Focused RED:** `SlashCooldownAxisMismatchRedSuite` compiles and fails 1/1 at ordinal anchor 599 versus stored epoch-progress expiry 142; SHA-256 `7fd927373ece8ccc221344eed83c0b085c59003f7cbe2ea68ceaba4b0948a766`. |
...
| FOLLOW-008C | Replace after the first short acquisition capture, during each unlocked snapshot/MPT/semantic/anchor readback, and immediately before the final descriptor CAS. The final CAS returns stale/retry and no mixed lease is minted; no finality lock spans image verification. **Partial dark mechanism GREEN only:** `CanonicalPhase2LeaseAcquirerSuite` covers the generic effect order plus replacement/recovery/descendant/error/cancellation interleavings, but its identity-equal fixture is not an authenticated snapshot/MPT/semantic/anchor reader, and no production boundary or caller exists. |
...
| REC-004 | Durable tower and historical-SMT enumeration rejects the entire image on a malformed, truncated, oversized, noncanonical, duplicate, out-of-range, or trailing-byte key, and rejects a negative replay lag or overflowing derived version before replay mutation; it never `flatMap`s/filter-drops an entry and continues with reduced history. Inject each defect plus failure/cancellation at every subsequent version-fold step across restart, compaction, shallow/deep density reorg, proof serving, and catch-up. Separately inject failure/cancellation during isolated steady-state fork preparation, after recovery-KV insert, before/inside durable commit, and while cancellation races the masked final publication: readers observe the complete prior or complete next in-process generation only, later append cannot extend stale live state, and completed durable recovery/replay plus the next append equal a clean uninterrupted build byte-for-byte. Power loss still requires an all-or-none durable publication receipt. The node enters authenticated recovery before publishing a root, tower proof, finality phase, or eligibility result. |
...
the prior complete tree (`HistoricalCommitmentSmtStore.scala:289-327,485-529`;
`HistoricalCommitmentSmtStoreSuite.scala:588-883`). Steady-state append prepares
an isolated structural-sharing SMT fork, savepoints and rolls back the recovery
KV before commit, then masks `durable.commit` plus one
`Published(versioned,cursor)` swap under the store lock
(`HistoricalCommitmentSmtStore.scala:237-276,330-450`). Fixed eight-byte keys
avoid enumerating retained keys on ordinary append, and the SMT owns mutable value
bytes across construction, reads, proofs, verified output, and forks. Focused
failure/cancellation, retry/reopen, operation-count, policy, and alias tests prove
old-or-new publication within one live process
...
`VersionedSmtSuite.scala:175-235`). This does not close `REC-004`: `MptStore.commit`
has no synchronous persistence receipt, so power-loss/crash atomicity remains
`STOR-01`; exact-hash density-reorg replacement and authenticated production
boot/branch wiring remain open. The former detached
...
| NET-007 | Fresh genesis, cold restart, and rollback expose zero remote consensus mutation before root-verified local state and exact chain seed succeed. Pre-state/pre-seed success and input release reject; failure/cancellation always wakes Main; ChainSync returns `UNAVAILABLE`. Target ordering is seed -> generic consumer/event sinks/Main listeners -> activate both subscription lanes -> exact current-generation acknowledgements -> boot-mode/catch-up policy -> `Ready`. Hostile early injection changes no queue, mempool, tip, MPT, committee, shard buffer, fraud pool, KES state, or served snapshot. Resource restart creates one fresh closed gate. |
| NET-008 | DAG, allow-spend, and token-lock gossip authenticates and structurally bounds the original signed block before enqueue. Per-peer/topic and global byte/item limits bound invalid and compromised-key floods; no rejected input is re-signed by GL0. Every admitted native GL1 transition is still universally executed by GL0 before state use. |
| NET-008A | Retain the native DAG producer/follower execution witness and add equivalent producer-plus-independent-follower parity for `AllowSpendEvent` and `TokenLockEvent`. From the same exact proposal parent, both sides execute the same signed native input and reproduce byte-identical acceptance, economic state, MPT entries, and root. A follower with divergent parent state or a mutated native input rejects before adoption or exposes a root mismatch. These are permanent universal `GL1 -> GL0` regressions and never enter a sharded-CL1 certificate/diff path. |
...
| NET-014 | After O-18 is owner-dispositioned, one canonical descriptor/content-addressed chunk contract is enforced across the current 20 MiB application aggregate, 1 MiB GossipSub, 4 MiB gRPC receive defaults, 16 MiB ChainSync, disk, compression, and decompression. Freeze active-era per-family canonical-uncompressed, compressed-envelope, structural-cardinality, chunk-count, and aggregate-retrieval maxima. The theoretically schema-valid v4 set has no finite payload maximum; a future migration inventories the exact selected source chain plus explicit headroom. Boundary vectors prove exactly-at-limit success and `limit+1` typed rejection, and a high-ratio payload stops during bounded streaming decode without materializing the full expansion. Gossip carries only the ratified bounded descriptor; exact length/content/chunk hashes, index/count, compression domain, duplicate/conflict handling, multi-peer retry, outbox retry, restart, and reassembly are verified before publication or execution. Any artifact permitted by the ratified application budget succeeds by authenticated bounded pull and can never be pushed as one oversized envelope. |
...
| PERM-006 | Exact-parent historical stake capture proves complete ancestry, reproduces supplied base/parent roots under their ordinal hashers, and reads period `N-2` using the exact boundary write-era key. Producer and follower consume only that capability; unavailable mature history defers before signing/validation. A hash-era transition migrates/rekeys field 20 atomically and removal reproduces the original physical key. **Current reader is dark and unwired; field-20 value lacks `EtaPeriod`; ACTIVATION NO-GO** (`HistoricalStakeReader.scala:18-40,60-120,152-161`). |
...
`GENESIS-ECON-001` and `GENESIS-SECRET-001` qualify only the fresh greenfield
ceremony. They do not satisfy `MIG-*`; the future upstream-v4 exporter,
historical decoder, transform, conservation report, and ScodecV1 target manifest
remain open.
...
|---|---|---|
| W0 | P0 decisions, architecture guard, finding ledger, RED harness | O-01 through O-17, O-20, O-22, and O-23 are direction-ratified. O-18 and O-21 plus S2-ORDER-01..03, P7-CPREF-01..06, O13-A1 through O13-F including A3's exact schema/disposition, TL-EXP-1..4, and the reward-algorithm choices remain owner stop-lines for their dependent work. O-19 is deferred with launch-profile-specific P12 and does not block greenfield runtime. Dependent work remains blocked wherever a register or focused packet names an owner, implementation, schema, parameter, research, RED-vector, or proof gate. |
...
| W2A | Dark P6 phase/lease/tower schemas and RED cores; dark P5 DA/intake schemas, REDs, and local custody primitives; P4.2 O-11/rooted roster contract; P4.3 exact historical resolver; PSIG boundaries; P7.0 shared exact-context/checkpoint-reference schema plus pure diff/root and ROOT-010 witness contracts/REDs | No runtime tower/eligibility activates before complete-root and executable O-11 historical authority. No live DA/intake byte wiring activates before O-18. P7 checkpoint-parent bytes remain unimplemented before `P7-CPREF-01..06`. Genesis consumers pass the immutable safety cut; all authority-bearing consumers remain blocked by their exact history/root/phase gates. |
| W2B | P6 atomic integration/recovery hooks and exact Phase-2 bootstrap-bundle API; P4.4 historical verification over P7.0 exact references; then remaining P7 one-checkpoint execution/result integration including ROOT-010's exact replay witness | Phase/reorg events, exact refs, bundle payload identity, view-preimage hashes, and explicit ML0 populations are load-bearing before checkpoint or follower base/anchor merge; ordinal-only, latest-after-finalized, wrong-root, missing-witness, and proof-subset-population paths are RED. |
| W3A | P4.5/P4.6 consumer/fixture migration plus P8 shard execution, positive watchtower coverage, challenge quarantine/exceptional replay verdict, and pure adoption primitive | Cross-consumer key parity, replay, one-outstanding batching, positive coverage, challenge-triggered verdict, and pure zero-recreation adoption tests pass. Unchallenged ordinary checkpoints are not universally replayed. This wave does not perform the E9 live GSAM cutover or economic slash delivery. |
...
| W4B | P8 pure adoption primitive feeds E9's sole live GSAM cutover and S6/S7; P10/P11 then close S8 reorg/recovery; only afterward does E11 integrate portable evidence with slash/bounty delivery and rollback/reorg effects | No checkpoint-derived value activates until S5's ordinary positive-coverage gate and challenge-quarantine/exceptional-verdict path plus E9's adoption/settlement checks pass. Unchallenged ordinary checkpoints are not universally replayed. Later slash effects preserve a challenge verdict under restart/reorg, debit actual backing, and cannot turn unavailable history or local configuration into guilt. |
| W4C | Deferred P12 offline migration against the frozen target schema for a source-network hard-fork launch profile, after an explicit O-19 owner disposition | Independent transform/root/conservation gates pass; deferral of P12 does not block a greenfield launch or remove the future migration capability. |
...
P13 has no authorship in P1-P12 closing changes. Release requires every applicable
catalog test; omitting one requires a proved non-applicability result. It also
requires zero unowned/open CRITICAL/HIGH findings, exactly one
active GL0 chain engine, zero blind state-signature APIs, zero universal ordinary-
adopter CL1 replay (while preserving universal native GL1/global-kernel
execution), zero GSI authority/fallback, and one active ScodecV1 genesis era.
`PARAM-001`, zero consensus-validity/root/phase effect from receiver-local
configuration or wall clock, and `SHARD-C-005` at `numShards=1` are unconditional
release gates. Release also requires a source re-audit of every
sign/adopt/phase/serve/recover path.
