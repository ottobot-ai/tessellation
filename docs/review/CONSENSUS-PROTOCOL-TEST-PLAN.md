...
| State/storage | MPT keys/values, finality journal, branch undo, checkpoint/diff/input retention, nullifier/inbox/outbox records, and atomicity contract. |
...
| P7 Diff/root/checkpoint | shared checkpoint schema/diff/root model/codecs | P1/P2/P4/P5/P6 refs | Complete root coverage and exact apply-diff vectors. |
...
| P9 Cross-metagraph settlement | global pure kernel plus owned GL0 integration | P2/P6/P8 | Allow-spend exact-once across shards, reorg, restart, and acknowledgement. |
| P10 Followers/MPT/GSI | one layer/module slice per agent | P6/P9 typed APIs | Exact-hash rollback and zero GSI authority/fallback. |
| P11 Transport/recovery/bootstrap | sidecar/JVM boundary, exact state recovery | contracts early; semantics after P6-P10 | Bounded adversarial transport and exact crash/catch-up/bootstrap. |
...
| W2A | P6 FinalityGate/tower core, P5 DA/intake runtime, P4.2 O-11/rooted roster contract, P4.3 exact historical resolver, PSIG GL0/ML0/GL1 signing boundaries; P7 pure diff/root/codecs against frozen interfaces | Genesis consumers pass the immutable safety cut. Runtime work proceeds only after O-11; then same-ordinal wrong hash/root, historical rotation, recovery, identity, phase, and non-shard signing gates pass. |
...
| W3B | P9 allow-spend global settlement | E8 interfaces plus exact-once/CAS/mirror-overlay gates pass. |
| W4A | P10 follower/GSI/correction slices and P11 recovery/transport | P2 reorg, protocol correction rebase, tower restart, and exact catch-up pass; target state schema and GSI denylist freeze. |
