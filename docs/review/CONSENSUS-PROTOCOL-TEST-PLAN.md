...
|---|---|---|---|
| P4.0 Atomic authority inventory (`PARTIAL`: atomic API plus raw and semantic source tripwires landed; historical qualification open) | One paired KES+VRF API; transport keys are evidence only; split mutable registries and direct-key consumers denied by inventory. Read-only KES/VRF projections may derive only from the atomic pair. The raw tripwire scans split factories, primitive references, and direct constructors. `OPERATOR-CONSENSUS-KEY-SEMANTIC-MANIFEST.tsv` separately inventories reviewed lexical spellings and file/counts for higher-level draw, eligibility, duty, signing, proof, tower, and evidence calls, including blocked absent roles, and pins uncommented/unquoted negative-vector test-shaped declaration names. Same-file line motion/substitution is not detected, and neither inventory is an AST, symbol-resolution, call-graph, or branch-historical proof. | P1 identity bytes. | `KEYREG-001`, `KEYREG-005`, `KEYREG-006`, `KEYREG-011`; checked semantic inventory has zero unapproved production key/sortition call sites and every allowlisted path passes branch-historical vectors. |
...
| P4.6 Fixture migration and release guard (`PARTIAL`: fixture migration plus textual and semantic tripwires landed; generated historical qualification open) | Every consensus fixture commits the complete genesis pair/population or registers the pair in the exact canonical N-2 view before use; arbitrary keys remain only in low-level crypto implementation tests. `CommitteeSortitionSuite`, `CommitteeShardSortitionSuite`, and `EligibilityCheckerSuite` use loader-validated period-zero pairs, including registered wrong-key controls and repeated canonical-input statistical draws. The semantic manifest anchors existing unregistered, wrong-era, wrong-VRF, and wrong-KES no-side-effect tests but does not replace the generated branch/era cross-consumer matrix. | P4.5 APIs frozen; may migrate independent fixture families earlier. | `KEYREG-006`, `KEYREG-011`; generated-unregistered key fails before every draw/proof/sign/attest/store/adopt/slash side effect and the inventory artifact reports zero exceptions without an explicit primitive rationale. |
