...
| Checkpoint result | Exact Phase-2 base, input commitment, complete root/write scope, canonical diff, extracted global intents, execution threshold, separately domain-separated positive replay coverage, and challenge material. |
...
| PF Finality model | pure phase/Avalanche/density model and checker | P0 finality answers | Reference transcripts, counterexample report, and parameterized assumptions. |
| P4 Operator registry/stake/KES/evidence | immutable-genesis population cut; unified operator-key record; historical activation, roster, backing, committee, crypto, and evidence schemas/tests | P1 identity primitives; O-11 before runtime eligibility; runtime branch lookup after P6 exact-ref interface; checkpoint migration after P7 exact refs | Rooted genesis pair/population binding, N-2 period-index rotation on an exact branch, cross-consumer key parity, backing, false-slash, exact-debit, and anchored-roster tests. Existing atomic registry/genesis commitment/resolver code is only a partial artifact. |
| P5 Lane/DA | envelope/registration/content commitment and bounded fetch | P0 lane answer, P1 | Currency and currency-with-data contract plus withholding/recovery tests. |
| P6 Durable FinalityGate/tower | node-shared finality store, tower/SMT, plus owned GL0 integration | P1/PF; economic release hooks after P2 oracle | Crash-consistent operational phase, load-bearing tower/SMT, arbitrary-depth density recovery, exact event outbox. |
| P7 Diff/root/checkpoint | shared checkpoint schema/diff/root model/codecs plus complete-root ownership and strict physical-entry interface | P1/P2/P4/P5/P6 refs | ROOT-007 complete coverage, ROOT-008 strict key grammar, and exact apply-diff vectors. |
| P8 Shard execution/watchtower | sharding runtime plus owned GSAM adapter | P7 | Replay-before-sign, zero-replay ordinary adoption, staircase/anchor/challenge tests. |
...
| P11 Transport/recovery/bootstrap | sidecar/JVM boundary, root-invisible-load normalization, strict durable tower/SMT enumeration, exact state recovery | contracts early; semantics after P6-P10 | ROOT-009/REC-004 recovery parity plus bounded adversarial transport and exact crash/catch-up/bootstrap. |
...
select both keys from one preregistered record; it does not by itself decide
whether either key may be unchanged from the preceding record. Reuse and
rotation semantics remain an owner decision. Runtime records therefore remain
ineligible. `StakeRegistry`/`SharedServices` still admit receiver-local
seedlist/current-state population inputs. O-11's candidate atomic
population-plus-raw-weight boundary is explicitly PROPOSED, not a schema or
authority decision; registration, positive stake, and injected test rosters remain
insufficient.
...
| P4.2 Permissionless roster contract (`BLOCKED ON O-11`) | Canonical Sybil-resistance/membership rule with minimum backing, delegation/collateralization, activation, exit/unbond, slash/cooldown, immutable-genesis population, retention, and recovery semantics. Evaluate the proposed single boundary value whose sorted population keys and raw weights are authenticated together; do not freeze it until O-11 and P1 approve the rule and codec. Registration cannot create membership. | Owner ratifies O-11; P1 codecs/root ownership. | `KEYREG-013`, `PERM-*`, `PARAM-001`; self-signed registered outsider, key-only/stake-only/profile-only entry, live-peer/seedlist substitution, stake splitting, early unbond, mixed-branch roster/weight, and receiver-local population are RED. |
| P4.3 Exact historical activation (`PARTIAL MODEL`, runtime blocked) | Authenticated exact-parent `(ordinal,hash,stateRoot)` view; period `N` uses the `N-2` pair/roster/stake view and `N-1` eta; paired activation/rotation unwinds and refolds with density reorgs. | P4.2 and P6 exact-parent branch interface. | `KEYREG-002..005`, `KEYREG-007`, `KEYREG-008`, `CRYPTO-001`; same ordinal with sibling hash, correct claimed hash with sibling registry/root bytes, live-head fallback, and missing history reject/defer before side effects. |
| P4.4 Exact Phase-2 artifact references | Currency binary and checkpoint base/anchor schemas bind exact `(ordinal,hash,stateRoot)` Phase-2 evidence used for registry, eta, roster, and stake resolution. | P4.3; P6 phase proof; P7 schemas. | `KEYREG-006`, `KEYREG-007`, `KEYREG-010`, `SHARD-E-003A`, `SHARD-C-001B`, `FOLLOW-001`; same ordinal/wrong hash or root fails producer, signer, acceptance, reorg, and adjudication paths. |
...
The hard dependency is O-11 owner answers -> P1 canonical population/root codec ->
...
| S4 One-checkpoint execution | One outstanding checkpoint per shard containing multiple ordered per-MG segments, complete canonical diff/root, replay-backed execution signatures. | S1 economic oracle; S2 P2 refs; S3 inputs. | Producer and every signer reproduce exact output; `kQuorum-1` and every tamper reject; no second outstanding checkpoint can be built. |
...
paired records. The runtime gate remains blocked on O-11, an authenticated exact-
parent view, exact Phase-2 artifact references, and consumer/reorg/evidence
migration. A persisted runtime record or passing resolver unit test is not
runtime eligibility.
...
| ERA-002 | Test-only ScodecV2 activation covers O-2 through O+2, sibling branches across O, activation reorg, restart, catch-up, wrong-era/downgrade bytes, and a live authorization crossing O. |
...
| KEYREG-004 | Rotation vectors cover the last candidate under the old record and first candidate under the new record at exact eta-period/parent boundaries. Premature new keys, stale old keys, gaps, overlapping claims, replay after rotation, and KES-only or VRF-only rotation reject deterministically under the atomic-pair rule. A separately ratified vector decides whether re-registering the same complete pair is valid or rejected. |
...
| KEYREG-007 | Two branches at the same ordinal include, rotate, or orphan different records before and after activation. Candidate validation uses only an authenticated exact parent `(ordinal,hash,stateRoot)` view. A sibling hash, the requested hash paired with registry/GSI bytes from the sibling root, an ordinal-only lookup, and receiver-current canonical state all reject/defer before side effects. A density reorg atomically unwinds/refolds active and pending records. |
...
| ECON-F-001 | Fee debit/sink/refund and partial consume semantics are explicit and conserved. Cancel/expiry/consume priority matches the ratified rule. |
...
| DIFF-003 | Every allowed writable key changes the verified root. Economic active allow-spends field 7 is covered. `globalSnapshotSyncView` remains committed by the exact signed ML0 binary but cannot be emitted as a writable GL0 diff key or unrooted canonical field. |
...
| SHARD-C-001B | A producer cannot select an older anchor ordinal and its matching favorable epoch/committee merely because historical replay state is retained and `anchor <= containingOrdinal`. The proposal-parent exact Phase-2 `(ordinal,hash)` evidence and ratified freshness/lineage rule determine the only admissible anchor before epoch/eta/roster lookup. Current acceptance of a self-consistent old pair is a passing RED characterization, not valid target behavior. |
...
| XMG-003 | Consume versus cancel/expiry/refund/local spend and inbound delivery versus local spend follow the ratified total order after every input permutation. |
...
| LANE-004 | Pure opaque/data-only registration and payload reject when v1 scope disables it. If owner retains it, replace this with isolated no-economic-effect progression tests. |
...
| ROOT-008 | One shared strict physical-entry reader retains `(actualHexKey,rawBytes)`, consumes the entire bounded codec, derives the canonical key and scope from the decoded value, compares byte-exact key identity, and rejects duplicate logical identities before map/set construction. Generated negatives cover every Mg field, consumed nullifier, stake/collateral, token lock, slash/cooldown, price, and parameter partition, including wrong key/scope, two keys for one value, mixed/empty sets, malformed/trailing bytes, and token-lock `headOption` substitution. No `entries.values`, right-biased `toMap`/`SortedMap`, filter/drop, or `headOption` path can silently choose consensus state. |
...
| `FOLLOW-*`, `GSI-*` | P10 by module | `target/consensus-audit/P10/` |
| `REC-*`, `NET-*`, `PERM-*`, `LIGHT-*`, `RESOURCE-*` | P11 | `target/consensus-audit/P11/` |
...
|---|---|---|
| W0 | P0 decisions, architecture guard, finding ledger, RED harness | Every blocking decision answered or explicitly keeps dependent packet blocked. |
...
| W2A | P6 FinalityGate/tower core, P5 DA/intake runtime, P4.2 O-11/rooted roster contract, P4.3 exact historical resolver, PSIG GL0/ML0/GL1 signing boundaries; P7 pure diff/root/codecs and ROOT-007 root-ownership contract | Genesis consumers pass the immutable safety cut. Runtime work proceeds only after O-11; then same-ordinal wrong hash/root, historical rotation, recovery, identity, phase, non-shard signing, and complete-root ownership gates pass. |
| W2B | P6 atomic integration/recovery hooks, P4.4 exact Phase-2 key-context references, then P7 one-checkpoint execution/result integration | Phase/reorg events and exact `(ordinal,hash,stateRoot)` refs are load-bearing before checkpoint base/anchor merge; ordinal-only or wrong-root lookup is RED. |
...
| W3B | ROOT-008 shared strict reader; then parallel P9 XMG-013 nullifier and P10 PERM-005, WT-010, ECON-G-002/parser migrations alongside allow-spend global settlement | ROOT-007 and the strict-reader interface freeze first. Each disjoint migration passes its physical-key RED corpus; E8 interfaces plus exact-once/CAS/mirror-overlay gates pass. |
| W4A | P10 follower/GSI/correction slices and P11 ROOT-009/REC-004 recovery/transport normalization, followed by integrated GSAM/reorg tests at shard counts 1, 2, and K | P2 reorg, protocol correction rebase, root-invisible ingress stripping/rebuild, durable-key corruption handling, tower restart, and exact catch-up pass; target state schema and GSI denylist freeze. |
