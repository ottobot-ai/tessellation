...
| ECON-SLASH-001 | Full and partial slash debit or rewrite the exact backing lock for active and pending delegated/collateral encumbrances atomically with metadata, expiry index, bounty, cooldown, and evidence. Race same-round/prior-round withdrawal against adjudication: withdrawal cannot hide guilty principal in pending state and maturity cannot refund more than post-slash principal. Bounty is at most actual debit; supply/root equality survives restart and reorg. **CRITICAL RED/OPEN.** |
