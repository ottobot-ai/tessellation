...
| FOLLOW-008 | Let branch A qualify through decided `T_weight` or `k1` at ordinal N, then density-replace it with branch B whose hash at ordinal N has neither qualification. A's monotone ordinal watermark cannot qualify B: every B-anchored binary remains deferred until exact B evidence is verified under an unchanged branch-revision lease. Interleave best-tip replacement at every read/recheck boundary and require zero admission receipt, execution signature, checkpoint inclusion eligibility, economic read, or downstream release from mixed observations. |
...
| ROOT-008 | One shared strict physical-entry reader retains `(actualHexKey,rawBytes)`, consumes the entire bounded codec, derives the canonical key and scope from the decoded value, compares byte-exact key identity, and rejects duplicate logical identities before map/set construction. The landed transport-level suite must continue proving deterministic key order, immutable byte ownership/value equality, null/empty/malformed retention, nibble-prefix case-alias retention, prefix exclusion, prefix-only branch removal/upsert merge, and in-memory/filesystem parity; those cases alone do not pass this gate. Before partitioning, a whole-image preflight rejects uppercase, odd-length, invalid-digit, and nibble-equivalent aliased physical keys. Generated semantic negatives cover every Mg field, consumed nullifier, stake/collateral, token lock, slash/cooldown, price, and parameter partition, including wrong key/scope, two keys for one value, mixed/empty sets, malformed/trailing bytes, and token-lock `headOption` substitution. No `entries.values`, right-biased `toMap`/`SortedMap`, filter/drop, normalization, or `headOption` path can silently choose consensus state. |
