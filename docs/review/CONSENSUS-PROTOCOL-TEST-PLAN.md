...
| BOOT-001 | With exact Phase-2 F, a valid best tip H>F, and same-ordinal sibling responses, cold ML0 and GL1 accept only one bundle whose FinalityGate-resolved `(ordinal,hash,parent,stateRoot,evidence)` and content-addressed payload identity all name F. Reordering, splitting responses across peers, or observing F's ordinal before fetching H never installs H or mutates any follower store. |
...
| W2A | P6 FinalityGate/tower core, P5 DA/intake runtime, P4.2 O-11/rooted roster contract, P4.3 exact historical resolver, PSIG GL0/ML0/GL1 signing boundaries; P7 pure diff/root/codecs plus ROOT-007 root ownership and ROOT-010 field-32 witness contracts/RED vectors | Genesis consumers pass the immutable safety cut. Runtime work proceeds only after O-11; then same-ordinal wrong hash/root, historical rotation, recovery, identity, phase, non-shard signing, complete-root ownership, and staged-versus-backfilled field-32 divergence are explicit gates. |
...
| W4A | After the ROOT-010 replay witness is green, P10 removes field 32 from GL0 and completes proof-selected follower/GSI/correction slices; P11 wires the exact Phase-2 bundle into the shared transactional installer and runs BOOT-001..005, ROOT-009/010, and REC-004/005 before integrated GSAM/reorg tests at shard counts 1, 2, and K | Cold join, P2 reorg, protocol correction rebase, zero field-32 GL0 ingress/storage, complete native payload/slice proof, root-invisible ingress stripping/rebuild, durable-key corruption, tower restart, and exact catch-up are all-or-none; the target state schema/GSI denylist then freezes. |
