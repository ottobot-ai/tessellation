...
|---|---|---|---|
| P4.0 Atomic authority inventory (`PARTIAL`: atomic API, raw/semantic tripwires, frozen K7a resolver vectors, effect-obligation inventory, and bounded K7b-1 execution-attester evidence landed; remaining adapters and historical qualification open) | One paired KES+VRF API; transport keys are evidence only; split mutable registries and direct-key consumers denied by inventory. Read-only KES/VRF projections may derive only from the atomic pair. The raw tripwire scans split factories, primitive references, and direct constructors. `OPERATOR-CONSENSUS-KEY-SEMANTIC-MANIFEST.tsv` separately inventories reviewed lexical spellings and file/counts for higher-level draw, eligibility, duty, signing, proof, tower, and evidence calls, including blocked absent roles, and pins uncommented/unquoted negative-vector test-shaped declaration names. Its checked zero-effect obligation vocabulary covers draw, eta, possession, replay, both signature domains, aggregation/tracking, store/adopt/publish, and slash. Hard-coded reviewed evidence metadata currently qualifies only `KSEM-EXEC-008..011`: direct counters/sinks for eta/proof/KES/tracker/publish and source-dominated Ed signing after the concrete acceptance manager invokes its injected re-execution hook once and produces the private capability. The hook is a stub, so this exercises capability-before-sign ordering and does not prove CL1 replay semantics or economic correctness. Other obligations remain unproved. Same-file line motion/substitution is not detected, and neither inventory nor anchor mapping is an AST, symbol-resolution, call-graph, or branch-historical proof. | P1 identity bytes. | `KEYREG-001`, `KEYREG-005`, `KEYREG-006`, `KEYREG-010`, `KEYREG-011`; checked semantic inventory has zero unapproved production key/sortition call sites and every allowlisted path passes branch-historical vectors. |
