...
| Test ID | Required scenario and assertion |
|---|---|
| WT-000 | Before the O-22 activating era, honest construction emits no fraud-proof authority and every nonempty/old-shape artifact is rejected before GSAM, MPT, overlay, storage, serving, or finality mutation. Under field removal, JSON/other object decoders reject an explicit retired `fraudProofs` member and Scodec rejects old-shape/trailing bytes; no permissive unknown-field compatibility remains. A populated local pool and different watchtower/slash/bounty/cooldown HOCON values cannot change the artifact or root. Activation is exact-parent and reorg-aware; no dark local result is grandfathered. **Focused RED:** `O22FraudProofPreActivationContainmentRedSuite` compiles and fails 4/4 intended gates with zero errors: active producer/GSAM authority, retired JSON acceptance, retired Scodec acceptance, and current field-34 cooldown influence. SHA-256 `e21fe7bc5f3a98ebd1c97a0ad7d9384fc0397d5b1440be91e0b613a31dfd5cdc`. |
| WT-000A | Retired `fraudProofs` JSON is rejected when empty, populated, `null`, or malformed in direct and `Signed.value` decoding. The same strict decoder boundary rejects Brotli snapshot files, combined-checkpoint pairs, ML0 global-state wrappers, direct/streaming HTTP responses, GossipSub, and ChainSync payloads before signature/storage/state advancement. Adding the retired member to an otherwise correctly signed new-shape artifact cannot survive decode-and-rehash. Complete Scodec rejects old empty/nonempty 27-field and arbitrary trailing bytes, while the isolated upstream `GlobalIncrementalSnapshotV1` Kryo disk reader still promotes legitimate upstream-v4 data. |
...
|---|---|---|
| W0 | P0 decisions, architecture guard, finding ledger, RED harness | O-01 through O-17 owner directions are ratified; O-18 transport/DA bytes, O-19 migration policy, O-20 field-34 slash records, O-21 optimistic decision evidence, O-22 fraud-proof activation/adjudication, and O-23 slash liability/bond tranches await owner responses. Dependent work remains blocked wherever the register names an owner, implementation, schema, parameter, research, RED-vector, or proof gate. |
