...
| SHARD-C-012A | **Partial containment GREEN; recovery OPEN.** Missing local GL0 lineage generation defers before checkpoint replay/sign/publish. Replacement after replay but before signing emits no validity signature. Replacement before a held retry clears the volatile memo, emits no bytes, and forced ABA cannot revive it; descendant extension preserves exact-byte retry. If the orphan checkpoint is already the local shard tip, the producer fail-stops instead of clearing/reminting it. The test does not atomically bind the separately read lineage and execution base, revoke copies already held by execution attesters, supply authenticated old/new/MRCA reorg evidence, replace the shard tip/frontiers/anchor, close the final check-to-publish race, or provide restart durability, so it closes neither `SHARD-C-012`, `SHARD-C-008`, `FOLLOW-008N`, nor FIN-14. |
