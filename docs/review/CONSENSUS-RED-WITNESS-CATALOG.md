...
| `io.constellationnetwork.dag.l0.infrastructure.rewards.GlobalDelegatedRewardsMathDeterminismRedSuite` | `ECO-17`: identical consensus reward inputs produce one integer amount on every supported JVM/CPU implementation and fail closed under every admitted numeric input. | 2 cases: 1 failed (three validator-accepted Testnet updates produce a current price whose Java-permitted elementary-function results differ by one minted datum and one rooted protocol-wallet balance datum), 0 errors, 1 passed (admitted fractional-power overflow aborts). | `b375143b716af846ba72f48bf57fd9daea8352451ad16c0138a6781379cb04f6` | The witness uses live pre-`1049436` Testnet emission constants and the real two-window price promotion. `StrictMath` is a second Java-permitted result path, not the proposed replacement curve. | Freeze the ordinal-zero normative curve, exact rooted input bounds, zero-price semantics, and approximation/error/resource rules; replace binary64 transcendental authority with a proved deterministic interval implementation and cross-platform golden vectors. |
