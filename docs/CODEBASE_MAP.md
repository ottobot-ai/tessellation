...
**Consensus phases**: CollectingFacilities → CollectingProposals → CollectingSignatures → CollectingBinarySignatures → Finished
...
5. Rewards distributed (classic or delegated based on epoch)
...
**To modify consensus**:
1. FSM states in `node-shared/infrastructure/consensus/state/`
2. Phase logic in `node-shared/infrastructure/consensus/engine/`
3. Declarations in `node-shared/infrastructure/consensus/declaration.scala`
