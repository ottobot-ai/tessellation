...
| Queue | Was | Now |
|---|---|---|
| `SharedQueues.rumorQueue` | `Queue.unbounded[Hashed[RumorRaw]]` (circe.Json) | `Queue.bounded(16384)` + `parEvalMap(N)` validation in GossipDaemon + **tryOffer-drop** at the two peer-receive offer sites (SidecarRumorBridge, ConsensusRoutes/push-rumor) so the bound can't migrate upstream; blocking offer kept for local `Gossip.spread`. Gossip is round-replayed → drop-under-overload is correct + self-healing. |
...
1. **`GossipStream` sidecar bridge queue** — `Queue.unbounded[Option[GossipMessage]]` (GossipStream.scala:23). The layer directly upstream of rumorQueue; protobuf bytes (smaller than parsed Json, but the same inbound firehose). The tryOffer-drop at the rumor bridge (this session) prevents it accumulating *given the current immediate consumer*, but it should be bounded with backpressure to the gRPC `request(n)` observer to be robust. **Fix: bound + gRPC flow-control.**
...
6. `ConsensusEventLoop` command queue — `Queue.unbounded[ConsensusCommand]` (small ADT); already has a `dag_consensus_command_queue_size` gauge. **Fix optional: bound + the gauge already gives visibility.**
