...
| post-S2 (shard-convergence freeze, ord-16 then ord-26) | shard chain frozen at checkpoint-ord 16 (then 26 after the first fix); `embed-none matched=false` forever (the MG's adopt-tip ∉ any window `parentRefs`); producer `awaiting-embed` (`pipelineDepth=1`) ~1.5h while ml0 keeps producing | window anchor = **finalized base** (S2, depth-k lagged) vs gl0 embed-match (`pick`) keys on the **adopt tip** (ahead of finalized): per-MG *staggered* adoption (DEFER-ANCHOR advances the shard ord while deferring an MG) + a sub-second post-adoption gossip race let the producer mint a **stale re-include** — sole-MG window ends AT the adopt tip, nothing past it ⇒ gl0 can't embed ⇒ bestTip > adoptedShardOrd ⇒ pipeline freeze | fixed: **producer NEWNESS GATE** (`ShardCheckpointProducer.produceInner`) — include an MG only if its base-anchored window holds a binary *past* the gate reference. **Gate reference = the chain-wide per-MG checkpoint frontier** (`ShardChainStore.lastCheckpointedPerMgTip` — the latest binary minted per MG across the noteAnchor-followed bestTip ancestry), NOT `lastNGlobalSnapshotStorage.getCombined`. getCombined (latest *produced* GSI) **LAGS** the in-flight per-MG adoptions, so it sits BEHIND the adopt tip and let a stale re-include slip the gate (the **ord-26 re-freeze**, first fix `4a251eb0`). The frontier is reorg-safe and provably **≥ gl0's adopt tip** (gl0 only adopts checkpoints this chain minted, as a prefix of the ancestry), so "window past frontier" ⟹ "window past adopt tip" ⟹ embed-match always continues — the mint-unembeddable freeze is structurally impossible. §4-neutral (window content/anchor/diff-prior all unchanged — gate changes *inclusion* only); single-leader whether-to-mint policy (no split). Distinct from run-27d/e: that omitted a *derivable-new* MG (defer-whole-checkpoint); this omits a *no-new-content* MG (cannot regress it). |
