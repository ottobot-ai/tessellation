...
| post-S2 (shard-convergence freeze) | shard chain frozen at checkpoint-ord 16; `embed-none matched=false` forever (both MGs' adopt-tip ∉ any window `parentRefs`); producer `awaiting-embed` (`pipelineDepth=1`) ~1.5h while ml0 keeps producing | window anchor = **finalized base** (S2, depth-k lagged) vs gl0 embed-match (`pick`) keys on the **adopt tip** (ahead of finalized): per-MG *staggered* adoption (DEFER-ANCHOR advances the shard ord while deferring an MG) + a 0.6s post-adoption gossip race let the producer mint a **stale re-include** — sole-MG window ends AT the adopt tip, nothing past it ⇒ gl0 can't embed ⇒ bestTip(16) > adoptedShardOrd(15) ⇒ pipeline freeze (never mints ord 17 with the now-available content) | fixed: **producer NEWNESS GATE** (`ShardCheckpointProducer.produceInner`) — include an MG only if its base-anchored window holds a binary *past* gl0's adopt tip (`adoptedPerMgTip` = latest-produced GSI `lastStateChannelSnapshotHashes`, the SAME tip `pick` matches). §4-neutral (window content/anchor/diff-prior all unchanged — gate changes *inclusion* only); single-leader whether-to-mint policy (no split). Distinct from run-27d/e: that omitted a *derivable-new* MG (defer-whole-checkpoint); this omits a *no-new-content* MG (cannot regress it). |
