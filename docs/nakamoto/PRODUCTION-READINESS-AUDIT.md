...
> **CORRECTION (2026-06-30):** several sharded-security items below are now BUILT (`8ac7ce04f..0b7902d69`) and supersede their 2026-06-09 status. **GAP-1 / §2 item 1** (member verify-by-re-execution): an ALWAYS-RUN `watchtowerReExec` re-derives every MG's per-MG root on the quorum-accepted path and disputes via a real `FraudProofEnvelope`. **§2 item 2** (committee-VRF check): real `verifyVrf` / `CommitteeSortition.verifyShardMembership` (`verifyVrfStructural` is now only the registry-absent fallback). **§2 item 3 — INVALIDITY TIER ONLY:** an upheld dispute writes a durable slash to the `Slashings` MPT partition (fieldId 34) via `InvalidStateProofEvidence` (equivocation / non-participation sink status UNCHANGED). Proof transport + PIN-1 component roots + `ConsumedAllowSpends` (33) are wired. **Caveat:** re-execution exists but is not yet the PRIMARY token-model adoption gate — an ml0 authoritative-balance override still masks it; removing it is the live task. See CURRENCY-APP-TOKEN-ENFORCEMENT.md + SHARDING-PRODUCTION-READINESS-PLAN.md.
...
| 2 | ~~**VRF committee check is structural-only** (length ≥ 80 bytes, no crypto)~~ **[2026-06-30: BUILT — real `verifyVrf` / `CommitteeSortition.verifyShardMembership`; the structural length check is now only the registry-absent fallback]** | ShardCheckpointGl0AcceptanceManager:526-530 | Slice 13: real `CommitteeSortition.verifyMembership` against registered VRF-VK. Mitigated meanwhile by the seedlist-derived membership SET check (:338), but sortition unpredictability is not cryptographically enforced. |
| 3 | **Slashing is detection-only** | `SlashableEvidenceValidator` complete (9 steps, step 7 already-slashed = `neverSlashed` stub); equivocation validator built but **uncalled**; ~~no consequence path (stake/eviction), no SlashableEvidence tx consumer~~ **[2026-06-30: the invalid-state-proof (watchtower) tier now writes a durable slash to `Slashings` (34) via `InvalidStateProofEvidence`; equivocation / non-participation sink still pending]** | Wire the sink: evidence tx type → GSAM consume → stake effect. Both per-epoch-committee safety (SHARD-SORTITION plan) and EPOCH-PARTICIPATION demotion are gated on this. |
...
| 10 | ~~**RebootstrapOrchestrator default-OFF** — divergent-self-finalize deadlock has no automatic recovery~~ **RESOLVED / SUPERSEDED** — now **default-ON**, so divergent-self-finalize self-heals automatically; SUPERSEDED-as-primary by density-past-k₁ deep reorg (Track-3 S3, flag `band-density-reorg-enabled`) and retained as a manual last-resort escape hatch. | `application.conf` `rebootstrap-enabled = true` (default; src `RebootstrapOrchestrator.scala`) | Done (default-ON). The `true→false` flip is HARD-GATED on S3+S4 e2e-green + a deep-fork sim (Track-3 S5); not yet flipped. |
...
- **DESIGN-AHEAD-OF-CODE (label them)**: EXECUTION-SHARDING-COMMITTEE-VERIFY (§7 member-verify contract ~~NOT implemented — GAP-1~~ **IMPLEMENTED 2026-06 — always-run watchtower re-exec**), SLASHING-DESIGN (~~no consequence path~~ **invalid-state-proof tier now durable — `Slashings` 34; equivocation/non-participation sink still pending**), KES10-WAVE-2 (intake wired, MPT write + receiver-side registry missing), EPOCH-PARTICIPATION (machinery unwired), BLS designs (code on `feature/bls-aggregate-sigs` branch, mainline blocked on BC 1.85 stable).
