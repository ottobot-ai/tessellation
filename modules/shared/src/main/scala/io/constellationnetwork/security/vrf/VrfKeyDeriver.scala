 ...
 * 
 * Uses SHA-512 with domain separation to derive a 32-byte Ed25519 seed from
 * the raw secp256k1 scalar bytes stored in tessellation's PKCS12 keystores.
 * 
 * This avoids introducing new key material — every node's VRF identity is
 * deterministically bound to their existing p12 key.
 */
object VrfKeyDeriver {
   ...
   * 
   * @param secp256k1PrivateKeyBytes raw 32-byte secp256k1 scalar
   * @return 32-byte Ed25519 seed suitable for EcVrf25519
   */
