...
/** Stateful interpreter wrapper around [[KesProduct]] that manages the read-once key lifecycle.
  ...
  *   - [[currentPeriod]] — read the currently active period (where "active" means "the next evolution from now should target a strictly
  *     greater period").
  *   - [[signAt]] — sign a message at the requested period; evolves the in-memory key first, persists the evolved state, then signs.
  *   - [[evolveTo]] — explicitly evolve to `period` without signing. The bytes corresponding to periods `< period` are destroyed.
  *
  * '''Period alignment''': per the consensus-epoch-staggering design notes, KES periods are aligned with eta rotation cadence. This algebra
...
trait OperationalKeyMakerAlgebra[F[_]] {
  ...
  /** Sign `message` at `period`. If the in-memory key is already at `period`, signs directly; otherwise evolves first (destroying the bytes
    * for periods `< period`) and persists.
    *
  ...
  /** Evolve to `period` and persist. No signature is produced. Useful for proactively burning the past on a fast-forward boundary (e.g.
    * just after an eta rotation).
    */
