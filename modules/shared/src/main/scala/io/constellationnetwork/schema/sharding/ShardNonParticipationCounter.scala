...
/** Per-(shard, peer, epoch) non-participation accumulator (slice 17 — see `docs/nakamoto/HIERARCHICAL-SHARD-CHECKPOINTS-DESIGN.md` §10.3).
  *
  ...
  *     `totalSlotsAsLeader` increments. If the peer fails to emit a checkpoint for that slot before the next slot rolls,
  *     `missedSlotsAsLeader` also increments.
  *
  ...
  *
  * '''Why `Long` for the counts.''' Per-epoch counts are bounded by `etaRotationSnapshots` (≈ 100 in v1) × `numShards` ≈ 400 in the
  * headline topology — well inside `Int`. We choose `Long` defensively against future epoch length changes and to match the wire shapes of
