...
/** gl0 → gl1 follow-slice TRANSPORT payload (Axis 2 — see `docs/nakamoto/GL1-INCLUSION-PROOF-FOLLOW-DESIGN.md`).
  *
  * What gl0 serves over HTTP for the latest-finalized consumed-field slice, and what a gl1 follower fetches. Per the locked "Transfer
  * model" (design point 1): the `MptOverlay` cannot read value-bytes at an arbitrary historical ordinal, so there is no per-ordinal
  * historical-diff fetch. Instead gl1 fetches the LATEST-FINALIZED full slice (as a [[ConsumedFieldDelta]] delta-from-empty — every
  * consumed-field entry an upsert, removals empty, projected from gl0's finalized `GlobalSnapshotInfo` by
  * [[io.constellationnetwork.node.shared.domain.nakamoto.GlobalFollowSliceService.latestSlice]]) and recompute-matches it against the
  * signed snapshot's `stateProof.<field>Proof` roots (field-root equality).
  *
