...
/** The per-shard contribution to gl0-derived state for one checkpoint cycle.
  ...
  *
  * Each map key is a metagraph address belonging to this shard (per the static assignment in §4); cross-shard MG addresses are NOT
  * included here — they surface in [[ShardCheckpoint.emittedReceipts]] as [[CrossShardReceipt]] entries.
  *
  ...
  * canonical preimage hash includes this entire structure. Reordering or wrapping a field in `Option` silently changes the bytes — bump
  * the case class explicitly if the field set has to evolve.
  *
  ...
  *   per-MG sync-data updates produced from each MG's own snapshots (the per-MG slice of `MetagraphSyncManager.updateFromCurrencySnapshots`;
  *   the cross-MG `updateFromSpendActions` slice is §8 territory)
  */
...
object ShardDerivedStateDelta {
  ...
  /** Empty delta — no per-MG contributions this cycle. Used for liveness-ping checkpoints (T_alive trigger fires while no SC activity
    * has occurred since the last checkpoint; see [`SHARD-CHECKPOINT-GRANULARITY.md`](../../../../../../../../docs/nakamoto/SHARD-CHECKPOINT-GRANULARITY.md)
    * §2.4 Option C).
    */
