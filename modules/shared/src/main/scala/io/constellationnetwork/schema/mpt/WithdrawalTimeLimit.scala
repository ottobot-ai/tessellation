...
/** Node-wide MPT configuration: the epoch offset applied to node-collateral withdrawals to compute their expiry epoch. Supplied implicitly
  * alongside `StateProofSelector` because it shapes MPT write-path output (the node-collateral-withdrawal expiry index). When `None`, the
  * expiry index is not maintained — used at rebuild sites that pre-date the index and in tests that don't exercise it.
  *
