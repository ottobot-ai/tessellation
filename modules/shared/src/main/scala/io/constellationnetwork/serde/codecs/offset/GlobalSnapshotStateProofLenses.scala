...
/** Byte-offset lenses for `GlobalSnapshotStateProof` — the 17-field transitional state-proof.
  ...
  *
  * The three required hashes have fixed offsets and get direct `Fixed` lenses. The
  * `mptRoot` (final optional hash) requires a dynamic scan because everything between it and
  * offset 96 is variable-length depending on which prior optionals are present.
  *
  * This lens family is exactly what the MPT verification path will use: "given the bytes of a
  * snapshot's stateProof on disk, read just the mptRoot" — without decoding the 13 preceding
  * witness hashes or the merkle-root sub-record.
  */
object GlobalSnapshotStateProofLenses {
  ...
  /** Advance a cursor past one `Option[Hash]` field: 1 byte if absent, 33 bytes if present.
    * Returns the new cursor position, or `None` if the byte stream is truncated.
    */
  ...
  /** Advance past the `Option[MerkleRoot]` (first optional, 1 byte if absent or 37 bytes if present:
    * 1 discriminator + 4 leafCount + 32 hash).
    */
  ...
  /** Locate the `mptRoot` field (last optional Hash). Scans past the merkleRoot (field 4) and the
    * 12 optional-Hash fields before it, returning the byte slice occupied by the mptRoot's inner
    * hash if present, or `None` otherwise.
    */
  ...
  /** Convenience: `Option[Hash]` variant — returns `None` if mptRoot is absent, `Some(hash)` if
    * present, `Failure` if bytes are malformed.
    */
