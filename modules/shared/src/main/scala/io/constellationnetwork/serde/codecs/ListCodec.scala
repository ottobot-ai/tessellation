...
/** Generic scodec codec factory for `List[A]`.
  ...
  *
  * Unlike `NonEmptyListCodec.nonEmptyList`, the empty case is valid — `List` has no non-emptiness invariant. Unlike
  * `SortedSetCodec.sortedSet`, elements are preserved in insertion order (the contract a caller using `List` has chosen over `Set`).
  *
