...
/** Typed errors for serde operations. Sealed ADT — never matched by string.
  *
  * All three typeclasses (`ImmutableCodec`, `Persistable`, `Transmittable`) return `Either[SerdeError, A]` on decode /
  * `F[SerdeError \/ A]` on effectful decode. Encode paths raise on failure by construction (scodec `Attempt.Failure` at encode time means
  * the codec contract was violated — a bug, not a recoverable error).
  */
