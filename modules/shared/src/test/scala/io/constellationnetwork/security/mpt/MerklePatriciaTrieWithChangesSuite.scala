...
/** Property suite for `MerklePatriciaTrie.withChanges` (#56.3 in the MPT branch refactor).
  *
  * Core invariant: for any `base` trie and any `(upserts, removes)` delta, `base.withChanges(upserts, removes)` must produce a trie whose
  * root hash equals `MerklePatriciaTrie.makeParallelFromBytes(applyDelta(baseEntries, upserts, removes))`.
  *
  * If this property holds, the persistent-trie diff is consensus-equivalent to a fresh full build, which is the gate for using
  * `withChanges` as the `viewAt` materialization primitive in the branch overlay (#56.4).
  */
object MerklePatriciaTrieWithChangesSuite extends MutableIOSuite with Checkers {
  ...
  test("empty delta is identity (rootHash unchanged)") {
    case (j, h, _) =>
  ...
  test("empty delta on empty trie (no entries) is identity") {
    case (j, h, _) =>
      implicit val js: JsonSerializer[IO] = j
      implicit val hh: Hasher[IO] = h
      for {
  ...
  test("single insert into empty trie matches fresh build") {
    case (j, h, _) =>
      implicit val js: JsonSerializer[IO] = j
      implicit val hh: Hasher[IO] = h
      val key = Hex("ab" * 32)
  ...
  test("remove non-existent key on empty trie yields the empty trie") {
    case (j, h, _) =>
      implicit val js: JsonSerializer[IO] = j
      implicit val hh: Hasher[IO] = h
      for {
  ...
  test("remove the only key reduces trie to empty equivalent") {
    case (j, h, _) =>
      implicit val js: JsonSerializer[IO] = j
      implicit val hh: Hasher[IO] = h
      val key = Hex("12" * 32)
  ...
  test("remove-then-upsert in same delta yields the new value") {
    case (j, h, _) =>
      implicit val js: JsonSerializer[IO] = j
      implicit val hh: Hasher[IO] = h
      val key = Hex("ab" * 32)
  ...
  test("branch-to-extension collapse: remove one of two prefix-collision keys") {
    case (j, h, _) =>
      implicit val js: JsonSerializer[IO] = j
      implicit val hh: Hasher[IO] = h
      val sharedPrefix = "ab" * 16
  ...
  test("update-existing-key changes the root and matches fresh rebuild") {
    case (j, h, _) =>
      implicit val js: JsonSerializer[IO] = j
      implicit val hh: Hasher[IO] = h
      val key = Hex("ab" * 32)
      ...
      for {
        ...
        expect.same(expected.rootHash, result.rootHash) &&
          expect(base.rootHash =!= result.rootHash)
  }

  test("PROPERTY: withChanges == makeParallelFromBytes(applyDelta(base, delta))") {
    case (j, h, _) =>
      implicit val js: JsonSerializer[IO] = j
      implicit val hh: Hasher[IO] = h
      forall(sceneGen) {
  ...
  test("PROPERTY: applying removes-only matches fresh rebuild without those keys") {
    case (j, h, _) =>
      implicit val js: JsonSerializer[IO] = j
      implicit val hh: Hasher[IO] = h
      val gen = for {
  ...
  test("PROPERTY: applying upserts-only matches fresh rebuild with merged map") {
    case (j, h, _) =>
      implicit val js: JsonSerializer[IO] = j
      implicit val hh: Hasher[IO] = h
      val gen = for {
