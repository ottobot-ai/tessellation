...
/** Tests for the KES product (super × sub) composition.
  ...
  *   - "Multi-step verify" round-trip across multiple evolutions: every signature verifies against the VK at the
  *     period it was produced.
  *
  * '''Bifrost test-vector parity''' — same caveat as [[KesSumSuite]]: we do not port the byte-for-byte
  * `specOut_sk` / `specOut_sig` hex comparisons. The property-based tests provide equivalent functional coverage. The
  * specific hex test vectors in Bifrost's `KesProductSpec` (vectors 1-5, heights (1,1) through (3,3)) are exercised
  * here as functional roundtrips at the corresponding heights + step sequences, but without the byte-for-byte
  * comparison.
  */
object KesProductSuite extends SimpleIOSuite with Checkers {
  ...
  test("sign at step 0 + verify round-trips") {
    forall(Gen.zip(seedHexGen, payloadHexGen, heightGen)) { case (seed, msg, hs) =>
  ...
  test("verify fails with a different VK") {
    forall(Gen.zip(seedHexGen, seedHexGen, payloadHexGen, heightGen)) { case (s1, s2, msg, hs) =>
  ...
  test("verify fails on a different message") {
    forall(Gen.zip(seedHexGen, payloadHexGen, payloadHexGen, heightGen)) { case (seed, msg1, msg2, hs) =>
  ...
  test("same seed + height ⇒ identical VK (determinism)") {
    forall(Gen.zip(seedHexGen, heightGen)) { case (seed, hs) =>
      val kes = KesProduct.instance
  ...
  test("evolve + sign + verify at monotonic step sequence (multi-period)") {
    ...
    val cfgGen: Gen[((Int, Int), List[Int])] =
      Gen.zip(Gen.choose(1, 3), Gen.choose(1, 3)).flatMap { case (hSup, hSub) =>
    ...
    implicit val showCfg: Show[((Int, Int), List[Int])] = Show.show { case ((a, b), ss) =>
      s"h=($a,$b) steps=${ss.mkString(",")}"
    }

    forall(cfgGen) { case ((hSup, hSub), steps) =>
      val kes = KesProduct.instance
