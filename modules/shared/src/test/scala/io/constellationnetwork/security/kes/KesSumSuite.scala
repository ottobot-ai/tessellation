...
/** Tests for the KES sum composition.
  ...
  *
  * '''Bifrost test-vector parity''': Bifrost's `KesSumSpec` includes 5 spec-input test vectors with hex-encoded expected secret key trees
  * and signatures. We do NOT port the byte-for-byte `specOut_sk` / `specOut_sig` checks because:
  *
  *   1. Bifrost's evolve scrambles `seed` bytes with `random.nextBytes` on each call — meaning the persisted SK after evolve is not
  *      deterministic, even from the same input seed. The Bifrost spec checks tolerate this via the `KesTestHelper.areEqual` matcher which
  *      (per Bifrost's tests) ignores fields that have been scrambled by the eraser. Reproducing that helper is mechanical but adds ~150
  *      LOC of mostly-mirror code; the property tests below cover determinism of the VK and of sign/verify roundtripping at all evolved
  *      steps. 2. The "signature byte-for-byte equals expected hex" check in Bifrost is satisfied by determinism of the underlying Ed25519
  *      over the seed-derived signing key. The property-based test verifies the same property modulo signature length and verification.
  *
  * If we ever need byte-for-byte Bifrost interop (e.g. for a cross-chain VK migration), we'll add the test-vector helper at that point;
  * documenting the deferral here per [[feedback_epistemic_honesty]].
  */
object KesSumSuite extends SimpleIOSuite with Checkers {
  ...
  implicit private val showTuple3: Show[(Hex, Hex, Int)] = Show.show {
    case (s, m, h) =>
      s"seed=${s.shortValue} msg=${m.shortValue} h=$h"
  }
  implicit private val showTuple4: Show[(Hex, Hex, Hex, Int)] = Show.show {
    case (s1, s2, m, h) =>
      s"seed1=${s1.shortValue} seed2=${s2.shortValue} msg=${m.shortValue} h=$h"
  }
  implicit private val showTuple2: Show[(Hex, Int)] = Show.show {
    case (s, h) =>
