...
/** Locks in the per-field state-proof diff that [[StateProofValidator.validateProof]] logs on a mismatch (and that the live-follower
  * `GlobalSnapshotContextFunctions.perFieldRootDiffs` delegates to). The contract: name the diverging field as `field(c=<rebuilt>,l=<committed>)`
  * — `c` = the proof THIS node recomputed, `l` = the value the signed snapshot commits to — so a `StateProof Broken` line is read directly
  * instead of positionally diffing two 19-field proof dumps (the exact pain the catch-up wedge investigation hit).
  */
object StateProofComparisonSuite extends SimpleIOSuite {
  ...
  pureTest("identical proofs → no field diffs and equivalent") {
    expect(cmp.fieldDiffs(base, base).isEmpty) and
      expect(cmp.equivalent(base, base))
  }
  ...
  pureTest("historicalStakeSnapshots divergence is named, with c=/l= values, and breaks equivalence") {
    ...
    expect(diffs.size == 1) and
      expect(diffs.head.startsWith("historicalStakeSnapshots(c=")) and
      expect(diffs.head.contains("l=")) and
      expect(!cmp.equivalent(rebuilt, committed))
  }
  ...
  pureTest("currency infoRoot divergence is named via lastCurrencySnapshots") {
    ...
    val diffs = cmp.fieldDiffs(rebuilt, committed)
    expect(diffs.size == 1) and
      expect(diffs.head.startsWith("lastCurrencySnapshots(c="))
  }
  ...
  pureTest("multiple diverging fields are all named") {
    ...
    expect(diffs.exists(_.startsWith("balances(c="))) and
      expect(diffs.exists(_.startsWith("mptRoot(c="))) and
      expect(diffs.size == 2)
  }
  ...
  pureTest("smtRoot-only difference: surfaced by fieldDiffs (diagnostic) but ignored by equivalent") {
    ...
    expect(cmp.equivalent(a, b)) and
      expect(cmp.fieldDiffs(a, b).exists(_.startsWith("smtRoot(c=")))
  }
}
