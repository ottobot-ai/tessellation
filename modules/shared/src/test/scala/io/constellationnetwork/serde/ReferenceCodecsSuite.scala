...
import io.constellationnetwork.serde.codecs.instances.AllowSpendReferenceCodec.{codec => allowCodec, immutableCodec => allowImmutable}
import io.constellationnetwork.serde.codecs.instances.TokenLockReferenceCodec.{codec => lockCodec, immutableCodec => lockImmutable}
...
/** Structural + round-trip suite for the new `*Reference` codecs (AllowSpend / TokenLock).
  *
  * Both follow the 40-byte `[ordinal:8][hash:32]` layout pattern established by `TransactionReference` and `BlockReference`. The tests here
  * are intentionally brief — the shape is identical to the already-tested reference codecs.
  */
object ReferenceCodecsSuite extends FunSuite {
  ...
  test("AllowSpendReference: 40 bytes, ordinal-first, round-trips") {
    ...
    expect(bytes.length == 40L)
      .and(expect(bytes.take(8) == ByteVector.fromValidHex("0000000000000009")))
      .and(expect(bytes.fromImmutableBytes[AllowSpendReference](allowImmutable) == Right(sample)))
  ...
  test("TokenLockReference: 40 bytes, ordinal-first, round-trips") {
    ...
    expect(bytes.length == 40L)
      .and(expect(bytes.take(8) == ByteVector.fromValidHex("0000000000000011")))
      .and(expect(bytes.fromImmutableBytes[TokenLockReference](lockImmutable) == Right(sample)))
    val _ = lockCodec
    success
  }
