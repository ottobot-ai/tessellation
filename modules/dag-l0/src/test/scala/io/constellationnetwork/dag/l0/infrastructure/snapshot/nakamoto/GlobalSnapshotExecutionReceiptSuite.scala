      ...
      JsonSerializer.forAsync[IO].flatMap { implicit jsonSerializer =>
        Metrics.forAsync[IO](Seq.empty).use { implicit metrics =>
          ...
          for {
            ...
            producerKeyPair <- KeyPairGenerator.makeKeyPair[IO]
            (vrfSecretKey, vrfPublicKeyBytes) = VrfKeyDeriver.deriveVrfKeyPair(producerKeyPair)
            vrfProofBytes = eligibilityChecker.vrfProofForSlot(vrfSecretKey, slot, etaBytes)
            vrfOutputBytes <- IO.fromOption(EcVrf25519.default.vrfProofToHash(vrfProofBytes))(
              new IllegalStateException("valid test VRF proof did not derive an output")
            )
            producerId = io.constellationnetwork.schema.peer.PeerId.fromPublic(producerKeyPair.getPublic)
            ...
            operatorKeys = OperatorConsensusKeys(
              operatorPeerId = producerId,
              kes = KesRegistryEntry(VerificationKeyKesProduct(Array.fill(32)(0x61.toByte), step = 0), offset = 0L),
              vrfPublicKey = VrfPublicKey.fromBytes(vrfPublicKeyBytes),
            ...
            result <- NakamotoSnapshotValidator.validate[IO](
              ...
              lddConfig = LddConfig(1, 0, Ratio.One, Ratio.One),
