...
/** A reactive, back-pressured request channel for ChainSync.
  ...
  * The call site has the parent hash (from the child's payload), so a direct fire-and-forget `Async[F].start` with a `Ref[Set[Hash]]`
  * dedup works fine there.
  *
  ...
  *
  * This queue gives that site (and any future one like it) a cheap, idempotent way to say "someone should go get the chain around
  * ordinal N" without knowing anything about transports or peers. One drainer fiber services the queue, running a pluggable `worker`
  * that translates an ordinal request into concrete network action.
  *
  * ==Narrow by design==
  *
  ...
  *   - Dedup is "currently-pending" (`Ref[Set[Long]]`), not LRU/TTL. A noisy producer firing the same ordinal repeatedly collapses into
  *     one in-flight request; once the worker completes, the next offer is accepted. Good enough for a 5s-tick WARN site. If you need to
  ...
  *
  *   - '''Multiple job types sharing a worker budget.''' E.g. walk-back requests compete with opportunistic backfill for bandwidth and
  *     one should preempt the other. Replace `Long` with a sealed `Job` ADT; swap `Queue.bounded` for a priority structure
  ...
  *
  * Until then, any of those concerns can be layered on top of this queue as decorators (e.g. a `PriorityChainSyncQueue` that wraps this
  * one and routes into it). Generalize when you have a second concrete use, not before.
  */
...
object ChainSyncRequestQueue {
  ...
  /** Build a queue with a pluggable worker. The worker is responsible for one ordinal request — typically translating it into network
    ...
    *
    * @param worker invoked with the ordinal for each dequeued request; completion is awaited before the next item is pulled.
    * @param capacity queue capacity. 32 handles bursts of fork-finality warnings across the confirmation window without growing
    *                 unbounded; full queue → oldest-semantics drop at the producer.
    */
  def make[F[_]: Async](
    worker: Long => F[Unit],
    capacity: Int = 32
    ...
    Supervisor[F](await = false).evalMap { supervisor =>
      ...
      } yield new ChainSyncRequestQueue[F] {
        def request(ordinal: Long): F[Unit] =
  ...
  /** Default worker for the finality-walkback case.
    *
    * Strategy: ask a peer for its tip via `GetPeerTip`. If the peer's tip is at-or-past the ordinal we're asking about, hand the peer's
    * tip hash to `ChainSyncManager.requestMissing` — which triggers the existing hash-keyed fetch. When the tip snapshot arrives at
    ...
    *   - No bound on how large a gap to attempt. If peer tip is 10_000 ahead, this still triggers. In practice `NakamotoSyncDaemon` has
    *     its own tiering (Tier 1/2/3) that caps walk-back depth and falls back to `BackfillDaemon`.
    *   - No retry/backoff. If the peer is unresponsive, the worker fails; the next 5s finality-monitor tick re-offers.
    *
    * @param channel gRPC channel to the local sidecar (same one `ChainSyncManager` uses).
    * @param chainSyncManager existing hash-keyed fetcher; we piggyback on its dedup and fetch machinery.
    */
      ...
      Async[F]
        ...
        .flatMap { tip =>
          ...
          if (peerOrdinal < ordinal) {
            ...
            val peerTipHash = Hash(new String(tip.hash.toByteArray))
            logger.info(
