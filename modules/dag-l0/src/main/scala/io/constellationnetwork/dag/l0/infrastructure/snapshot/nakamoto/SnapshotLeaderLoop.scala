...
/** Pure attestation-based Nakamoto consensus loop.
  *
  * Replaces the entire BFT round system (Facility → Proposal → Signature → Finished).
  * No multi-party coordination. No rounds. No facilitators.
  *
  * Flow:
  *   1. Every 1s slot tick → evaluate VRF eligibility via LDD snowplow
  *   2. On win → drain mempool → call createProposalArtifact → sign → store → publish via sidecar
  *   3. Other validators receive via GossipSub → validate → broadcast TipAttestation
  *   4. TipTracker accumulates attestation weight → ≥ 2/3+1 = finalized
  *
  * The `activePoolSize` and `activePoolHash` are embedded in the SlotCertificate so verifiers
  * know what "2/3+1" means for that snapshot without needing global state.
  */
object SnapshotLeaderLoop {
  ...
  /** Run the pure attestation snapshot leader loop.
    ...
    *
    * @param consensusFns    existing createProposalArtifact (reused from BFT, not reimplemented)
    * @param snapshotStorage snapshot chain storage (head, prepend)
    * @param eventMempool    pending events (drain on win)
    * @param sidecarClient   GossipSub sidecar for publishing snapshots
    * @param tipTracker      attestation accumulator for finality
    * @param stakeRegistry   validator weights
    * @param hasherSelector  deterministic hashing (ordinal-aware)
    * @param keyPair         node's secp256k1 identity keypair
    * @param selfId          this node's PeerId
    * @param lddConfig       LDD snowplow parameters
    * @param slotsPerEpoch   slots per epoch for eta rotation (default 60)
    */
          ...
          for {
            ...
            _ <- result match {
              case Some((proof, vrfOutput)) =>
                onSlotWon(
                  stateRef, consensusFns, snapshotStorage, eventMempool, sidecarClient, tipTracker, stakeRegistry,
                  keyPair, selfId, vrfSeed, vrfPK, proof, vrfOutput,
                  currentSlot, slotGap, slotRefined, lddConfig, slotsPerEpoch, logger
                )
                ...
                for {
                  ...
                  _ <- Async[F].whenA(isNew) {
                    tipTracker.markFinalized(hash, slot) >>
                      ...
                      logger.info(s"✅ FINALIZED snapshot at slot ${slot.value.value} (hash=${hash.value.take(16)}..., weight=${"%.2f".format(weight)})")
                  }
                } yield ()
              case _ => Async[F].unit
            }
  ...
  private def onSlotWon[F[_]: Async: SecurityProvider: HasherSelector](
    ...
    for {
      ...
      etaHash = Hash(state.currentEta.map("%02x".format(_)).mkString)

      activePool <- stakeRegistry.activeValidators
      activePoolSize = activePool.size
      activePoolHashBytes = java.security.MessageDigest.getInstance("SHA-256")
        ...
        case Some((lastSigned, lastContext)) =>
          for {
            ...
            result <- consensusFns.createProposalArtifact(
              ...
              getGlobalSnapshotByOrdinal = ordinal => snapshotStorage.get(ordinal).flatMap {
