  ...
  def make[F[_]: Async](
    ...
    new StateChannelService[F] {
      ...
      def process(
        ...
        // for per-key reader lookups. The staking/owner balance lookups below use the reader.
        ...
        for {
          maybeCurrencyInfo <- reader.getCurrencySnapshotInfo(stateChannelOutput.address)
