...
object Download {
  ...
  /** Bind a fetched ML0 snapshot/context to the exact peer-selection tuple before it can become an observation or recovery base. */
  private[programs] def selectBoundLatest[F[_]: Async: Parallel: JsonSerializer: SecurityProvider](
    peerSelect: PeerSelect[F],
    fetch: L0Peer => F[(Signed[CurrencyIncrementalSnapshot], CurrencySnapshotInfo)]
  )(implicit hasher: Hasher[F], stateProofSelector: CurrencyStateProofSelector): F[
    (Signed[CurrencyIncrementalSnapshot], CurrencySnapshotInfo)
  ] =
    peerSelect.select.flatMap { selection =>
      fetch(selection.peer).flatMap { result =>
        val (snapshot, info) = result

        snapshot.toHashedWithSignatureCheck[F].flatMap(_.liftTo[F]).flatMap { hashed =>
          if (!(hashed.ordinal === selection.ordinal && hashed.hash === selection.hash))
            CurrencySelectedSnapshotMismatch(selection.ordinal, selection.hash, hashed.ordinal, hashed.hash)
              .raiseError[F, (Signed[CurrencyIncrementalSnapshot], CurrencySnapshotInfo)]
          else
            StateProofValidator.forCurrency[F].validate(hashed, info).flatMap {
              case Validated.Valid(_)     => result.pure[F]
              case Validated.Invalid(err) => err.raiseError[F, (Signed[CurrencyIncrementalSnapshot], CurrencySnapshotInfo)]
            }
        }
      }
    }
  ...
  )(implicit l0NodeContext: L0NodeContext[F], stateProofSelector: CurrencyStateProofSelector): Download[F, CurrencyIncrementalSnapshot] =
    new Download[F, CurrencyIncrementalSnapshot] {
      ...
      def start(implicit hasher: Hasher[F]): F[DownloadResult] = {
        ...
        retryingOnAllErrors[(Signed[CurrencyIncrementalSnapshot], CurrencySnapshotInfo)](
          ...
          selectBoundLatest(peerSelect, peer => p2pClient.currencySnapshot.getLatest.run(peer))
        }
      }
  ...
  case class CurrencySelectedSnapshotMismatch(
