...
/** Regression test for the iter-198std-v2 4-mg E2E stall (#145 reoccurrence at 4-mg scale).
  ...
  *   - First Download.observe completes (possibly with partial observation), registerForConsensus(K1) succeeds,
  *     setting observationKeyR := Some(K1) and FSM := Observing.
  *   - InitializeFromDownload exhausts its 20 fetch retries, escalating to the
  *     `ConsensusEventLoop` error handler which transitions FSM back to WaitingForDownload.
  *   - DownloadDaemon picks up the state event, downloads again, calls observe, then tries
  *     registerForConsensus(K2) with a fresh observation limit.
  *   - registerForConsensus delegates to storage.trySetObservationKey(K2) which uses
  *     `observationKeyR.getAndUpdate(_.orElse(K2.some))` semantics — it keeps the EXISTING Some(K1)
  *     and returns `false` (was-empty check fails).
  *   - The `.ifM(ifTrue, ifFalse)` branch raises:
  *     `Throwable("Registration failed: already registered at different key")`.
  *   - The daemon's retry loop catches it, sleeps 10s, re-reads the FSM state which is now stuck
  *     in WaitingForObserving (the tryModifyState already advanced it before registerForConsensus
  *     was even invoked). The daemon's `case other => abort` branch fires and the node freezes.
  *
  * Without the fix in ConsensusEventLoop.scala:208-260, the recovery path transitions the FSM
  * but does NOT reset the consensus storage observation key — so every subsequent attempt hits
  * the same collision.
  */
object InitFromDownloadRecoverySuite extends SimpleIOSuite {
  ...
  private def mkManager(storage: ConsensusStorage[IO, Event, Key, Artifact, Context, Status, Outcome, Kind])
  ...
  test("ConsensusManager.registerForConsensus raises when an old key blocks a new key without reset") {
    ...
    } yield expect(result.isLeft) and expect(
