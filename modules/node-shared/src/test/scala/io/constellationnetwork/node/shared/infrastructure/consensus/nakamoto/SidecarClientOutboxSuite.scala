...
/** Task #196 — exercises the JVM side of the durable-outbox surface:
  ...
  *
  * The Go sidecar's outbox lifecycle (Add → re-publish → Confirm → drop) is covered by the Go-side
  * outbox_test.go unit tests; the JVM job here is to make sure the algebra plumbs the right wire
  * shape across the gRPC boundary. We assert behaviour against a stub implementation that captures
  * every call — no real gRPC channel, no network — so the test stays fast and hermetic.
  */
object SidecarClientOutboxSuite extends SimpleIOSuite {

  /** In-memory implementation of `SidecarClientAlgebra` used by tests.
    *
    * Mirrors the Go sidecar's outbox semantics at the contract level: every publish lands in
    * `published(topic) -> List[msgID]`; `confirmFinalized(topic, ids)` drops matching ids and
    * returns the dropped count. Out-of-scope on this stub: the actual mesh publish, the
    * republish ticker — those are Go-side behaviours.
    */
  ...
  private def stubClient(ref: Ref[IO, StubState]): SidecarClientAlgebra[IO] = new SidecarClientAlgebra[IO] {
    ...
    def publishMetagraphAttestation(msg: MetagraphAttestation) =
      ref
        .update(s =>
          s.copy(metagraphAttestationPublished = msg.metagraphAddress :: s.metagraphAttestationPublished)
        )
  ...
  test("publishAllowSpendBlock captures the raw payload on the wire") {
    for {
      ...
      state <- ref.get
    } yield expect(state.asbPublished.size == 1)
  ...
  pureTest("OutboxTopic constants match the wire vocabulary the sidecar registers") {
    ...
    // TopicMetagraphBinary / TopicMetagraphAttestation in the Go sidecar.
    // A rename without updating both sides desyncs the outbox.
    expect.all(
