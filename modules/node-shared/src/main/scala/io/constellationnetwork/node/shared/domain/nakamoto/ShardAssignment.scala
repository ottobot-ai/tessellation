...
/** Static metagraph → shard assignment service (hierarchical-shard-checkpoints v1, §4.1 + §4.3).
  ...
  * the hex string — that's the SHA-256 of the hex of the SHA-256, and uniformity then depends on the hex-string distribution rather than the
  * digest distribution. Decoding the hex back to the 32 raw digest bytes gives us the same unbiased mod-uniform sample the digest itself
  * already provides, with no extra hashing pass. This is the convention asked for in the slice 3 task.
  *
  * '''Why `unsafeApply` on `ShardId`.''' The result of `(bi mod BigInt(numShards)).toInt` is guaranteed non-negative because `numShards > 0`
  * (`require`d at construction) and `BigInt(1, bytes)` yields a non-negative value (`signum = 1`). So the refinement check would be pure
  * overhead at this point — every call site has already mathematically established non-negativity.
  *
  * '''Late-registered metagraphs (§4.3).''' v1 does not re-balance when a metagraph registers mid-cluster: the deterministic mapping applies
  * at the new metagraph's registration ord and from then on. Load balance is therefore governed by the uniformity of the SHA-256 distribution
  * — for `M ≪ N_metagraphs` the law of large numbers gives roughly balanced shards. Pathological collisions are a v2 concern (consistent
  * hashing with virtual nodes / per-epoch redistribution).
  */
