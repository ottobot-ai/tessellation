...
object GlobalL0Service {
  ...
  def make[
    ...
    new GlobalL0Service[F] {
      ...
      //
      // The served byte file is gl0's SIGNED store (`mpt_snapshot_info_signed/<ordinal>`, written at every finalized ordinal and NEVER
      // pruned), so it should be PRESENT at the served combined-checkpoint ordinal (a subset of finalized ordinals) — the fast path is
      // expected to fire. The fetch can still fail transiently (peer not yet at that ordinal, network) → the route 404s/errors. So the
      // byte-route fetch is wrapped in `handleErrorWith` EXACTLY like the sibling fetches (`getLatestFollowSlice`/`pullGlobalSnapshot`): on
      // ANY error/404 we DO NOT raise (that would abort the consumer's resync
                ...
                .flatTap(_ =>
                  logger.info(
                    s"3c-A FAST-PATH fired: pulled gl0's signed MPT byte map (${entries.size} entries) at ord=${snapshot.value.ordinal.show}; " +
                      s"rebuilding the global MPT verbatim from signed bytes (NOT the legacy syncFromGlobalSnapshotInfo re-encode)"
                  )
                )
