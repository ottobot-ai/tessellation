...
object SharedQueues {
  ...
  /** Hard cap on buffered, not-yet-validated gossip rumors. 2026-06-10 (run b6zx4w20b): the queue was `unbounded`, and under the
    ...
    * death-thrash. Bounding prevents catastrophic growth. Local `Gossip.spread` falls back to a supervised blocking offer, while peer
    * receive sites use `tryOffer` and drop on overflow so the backlog cannot migrate upstream. Legacy gossip modes can recover through pull
    * rounds; Nakamoto mode deliberately skips those rounds, so an inbound drop is lossy until durable retry/outbox recovery lands. Sized so
    * the worst-case pinned-Json footprint stays well under the gl0 heap. Paired with the `parEvalMap` validation fan-out in GossipDaemon.
    *
    ...
    * ~10k inbound rumors (the tryOffer-drop path); one dropped rumor was an allow-spend that propagated only slowly in the legacy
    * round-replay deployment. With heap headroom proven, size the cap to ABSORB such bursts without dropping: 65536 × ~30 KB ≈ 2 GB, well
    * under the 12 GB heap and 6× the observed burst. Drops now signal genuine sustained overload, not a transient spike.
    */
