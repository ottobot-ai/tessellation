...
object FinalizedSnapshotReader {
  ...
  def nakamoto[F[_]: Async, S <: Snapshot, SI <: SnapshotInfo[_]](
    ...
    // Nakamoto reader (then the `mpt-entries` route 404s, identical to BFT). gl0 wiring passes a read-only `MptStateStorage` pointed at
    // the SAME `mpt_snapshot_info/<ordinal>` directory the producer persists to, so `readState(ordinal)` returns the exact signed bytes.
  ...
  /** 3c-A GLOBAL wiring helper. Builds a READ-ONLY `MptStateStorage` at `mptSnapshotInfoPath` — the SAME directory the gl0 MPT producer
    * persists `mpt_snapshot_info/<ordinal>` to — and constructs the Nakamoto reader with it so `/latest/combined/mpt-entries` can serve the
    * signed byte map. Effectful only because `MptStateStorage.make` creates/validates the directory; read-only access to the producer's
    * finality-gated files is race-free. Use this at the gl0 HTTP wiring site; non-global Nakamoto readers keep using [[nakamoto]] (which
