...
/** One entry in the §3 NIPoPoW superblock tower (paper §3.4): a finalized snapshot that hit super-level `µ` at ordinal `ordinal`,
  ...
  *
  * '''Level-0 NOT stored.''' Level-0 hits are implicit (every finalized snapshot is a level-0 hit by chain construction). The tower only
  * persists level-µ hits for µ ∈ {1..L-1}.
  */
@derive(eqv, order)
