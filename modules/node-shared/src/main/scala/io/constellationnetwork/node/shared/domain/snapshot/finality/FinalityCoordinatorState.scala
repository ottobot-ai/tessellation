...
/** Closed local coordinator mutations recorded in the immutable audit chain.
  * None of these values is portable fork-choice or finality evidence.
  */
...
/** Objective reason that normal coordinator mutation must stop.
  *
  * The reason is retained in an immutable recovery record rather than embedded in
  * the mutable coordinator head. None of these variants selects a winning branch.
  */
...
object RecoveryReason {
  ...
  final case class CoreConflict(expected: Option[ReleasedCorePointer], actual: Option[ReleasedCorePointer])
  ...
  final case class EffectConflict(kind: EffectKind, expected: EffectStateDigest, actual: EffectStateDigest)
      extends RecoveryReason
  ...
  /** Local startup verified the coordinator journal and audit chain, but could
    * not verify every dependency needed to expose the persisted Running head.
    * The digest commits to a canonically framed local diagnostic; it is not
    * portable fork-choice evidence and cannot select a branch.
    */
...
object CoordinatorMode {
  ...
  /** Absorbing for ordinary coordinator transitions. Recovery exit requires a
    * separate verified reconstruction operation, not a normal head mutation.
    */
  final case class RecoveryRequired(record: RecoveryRecordPointer) extends CoordinatorMode
}

/** Constant-size index into immutable effect manifests. Mutable receipt progress
  * lives exclusively in the independently checksummed [[FinalityEffectOutboxHead]],
  * so effect delivery cannot occupy or block the coordinator CAS used for urgent
  * core rollback. Pending work is never embedded in the coordinator head.
  */
...
/** Mutable coordinator journal head.
  *
  * Successful release is permitted only from `CoreStage.CoreApplied`; it clears
  * `active`, installs `released`, and advances `effects.tail/highGeneration`.
  * Restoration or abandonment never changes `released`. `RecoveryRequired` is
  * absorbing for ordinary transitions. The pure validator and durable store CAS
  * enforce these structural rules. CoreApplied, Released, and restoration
  * authority remain disabled until a future package-owned runtime can prove the
  * required exact state/effect readbacks.
  */
...
/** Exact mutable-head content excluding its audit pointer.
  *
  * The exclusion is deliberate: the immutable audit record commits to this
  * value, and the resulting audit pointer is then installed in the mutable head.
  * Including `auditTail` here would create a content-address cycle.
  */
...
/** Immutable append-only explanation of one coordinator-head replacement.
  *
  * `priorAudit` must equal the replaced head's `auditTail`; the new head must
  * equal `after` and point its `auditTail` at this record. The pure transition
  * validator owns those equalities.
  */
