...
object ShardCheckpointAcceptResult {
  ...
  /** Pre-checks pass but neither finality trigger qualifies yet — checkpoint is in-flight and will be re-evaluated at a later gl0 ord
    * (per §7.2 the `gl0AnchorOrdinal` allows the checkpoint to ride into N, N+1, N+2…). Caller skips inclusion this ord.
    */
  ...
  /** The checkpoint took the `T_depth1_shard`-only re-exec path and the local recomputed `mptRoot` for at least one metagraph differs
    * from what the committee signed. Per §10.2 "wrong-derivation": every committee signer of this checkpoint deviated from determinism
    * and is a slashable target. Caller (Slice 16+17) feeds `slashSigners` into the slash-evidence accumulator; for this slice we just
    * surface the list.
    */
...
/** gl0-side admission of [[ShardCheckpoint]]s — Slice 9 of `docs/nakamoto/HIERARCHICAL-SHARD-CHECKPOINTS-DESIGN.md` §7 (especially §7.3).
  *
  * Sits between the shard committee's checkpoint gossip and the gl0 leader's per-ord accept loop. For each candidate checkpoint included
  * in the next gl0 snapshot:
  *
  ...
  *      qualified `checkpoint.shardOrdinal` ⇒ fast `Accepted`. If only `tDepth1Shard` qualifies (degraded liveness — no quorum) ⇒
  *      re-exec path. Neither qualifies ⇒ `PendingMoreAttestations`.
  *
  *   1. '''Re-exec''' (T_depth1 only): for each MG in `checkpoint.includedSnapshots`, call the injectable `reExecuteDerivation` and
  *      compare the recomputed `mptRoot` byte-for-byte against `checkpoint.derivedStateDelta.perMetagraphMptRoots(mgAddr)`. All match ⇒
  *      `Accepted`. Any mismatch ⇒ `RejectedReExecutionMismatch` with the full list of committee signers as the slash target set
  *      (§10.2 — wrong-derivation evidence).
  *
  * '''Greenfield rule''' (per `[[feedback-greenfield-no-wire-compat]]`):
  *   - Fresh manager for the shard-checkpoint admission path. No compat ceremony with
  *     [[GlobalSnapshotStateChannelAcceptanceManager]] — Slice 13 rewires GSAM to consume this; this slice creates the manager
  *     standalone.
  *
  ...
  *
  * '''Decoupling rule''':
  *   - All shard-scope lookups are injected as callbacks (`finalityTriggers`, `chainStore`, `committeeMembership`,
  *     `reExecuteDerivation`). Tests can stub each path independently; production wires the live per-shard maps. The trait stays
  *     agnostic to where the per-shard state lives.
  */
trait ShardCheckpointGl0AcceptanceManager[F[_]] {
  ...
  /** gl0 leader runs this for each candidate [[ShardCheckpoint]] under consideration for inclusion in the next gl0 snapshot.
    *
    * Returns one of the [[ShardCheckpointAcceptResult]] variants; the caller branches on the variant for inclusion / deferral /
    * rejection (+ optional slashing evidence emission).
    */
...
object ShardCheckpointGl0AcceptanceManager {
  ...
  /** Construct a manager. All shard-scope dependencies are callbacks so the trait stays free of the concrete per-shard registries (those
    ...
    *   `shardId => F[Option[ShardFinalityTriggers[F]]]`. Returns `None` when the shard isn't tracked locally (e.g., we're the gl0
    *   leader but never started a `ShardChainStore` for this shard, or the chain hasn't bootstrapped). `None` ⇒ reject.
    * @param chainStore
    *   `shardId => F[Option[ShardChainStore[F]]]`. Carried for future use by the re-exec path's chain-walk dependency. Slice 9 does
    *   not consume it directly — `reExecuteDerivation` is the actual re-exec hook. Slice 13+ wiring may need it to verify that a
    ...
    *   `(shardId, epoch) => F[Set[PeerId]]`. The committee draw for this `(shard, epoch)`. Used in the pre-check to confirm each
    *   signer is actually a committee member at the claimed epoch. Empty set ⇒ any signer fails the membership pre-check ⇒ reject.
    * @param kTarget
    *   committee target size (`K_S` in design doc §5). Wired from `cfg.nakamoto.sharding.committeeKTarget`. v1 stable-σ rule treats
    *   every committee member as uniform 1/K_S; the threshold check inside `CommitteeSortition.verifyMembership` uses `K · σ` so this
    *   is the `K` factor.
    * @param selfPeerId
    *   this node's PeerId. Carried for diagnostic logging (so a slashing event surfaces which gl0 op spotted the deviation). NOT used
    *   to gate any acceptance logic — every honest gl0 op runs the same predicates and reaches the same outcome.
    * @param kesRegistry
    ...
    *   [[io.constellationnetwork.node.shared.domain.nakamoto.MetagraphCommitteeGate]]'s receiver path: registry-absent peers are
    *   accepted on the strength of the Ed25519 signature alone (Slice 10 will introduce runtime registration; this carve-out covers
    *   the bootstrap window).
    * @param reExecuteDerivation
    *   `(metagraphAddress, headBinary) => F[Hash]`. Called only on the T_depth1-only path. Slice 9 ships this injectable; production
    *   wiring (Slice 13) passes the closure that walks the per-MG chain and runs the existing `processCurrencySnapshots` derivation
    *   to compute the local `mptRoot`. For tests: stub the callback to return a known hash (matching or not matching the checkpoint
    *   delta) per scenario.
    */
    ...
    Async[F].pure {
      new ShardCheckpointGl0AcceptanceManager[F] {
        ...
        /** Pre-check pipeline: for each `CommitteeMemberSignature` in `checkpoint.committeeSignatures`, run four predicates in order
          ...
          *   1. KES product sig verifies under the registered master VK at the embedded tree-internal step (registry-absent ⇒ accept
          *      per the carve-out)
          *   1. VRF proof verifies as committee membership for `(checkpoint.shardId, checkpoint.epoch, ...)`
          *
          * Returns `Right(())` if every signer passes every predicate, `Left(reason)` on the first failure. Reason is a short
          * diagnostic string (signer index + which predicate failed) — diagnostic only, never parsed for control flow (per
                ...
                case (Right(()), (sig, idx)) =>
                  for {
                    ...
                    result <-
                      if (!membershipOk)
                        Async[F].pure(
                          Left(s"signer[$idx] peerId=${sig.peerId.value.value.take(16)}... not in committee for epoch=${checkpoint.epoch.value}")
                            : Either[String, Unit]
                        )
                      else
                        for {
                          ...
                          step2 <-
                            if (!edOk)
                              Async[F].pure(
                                Left(s"signer[$idx] peerId=${sig.peerId.value.value.take(16)}... Ed25519 sig verify failed")
                                  : Either[String, Unit]
                              )
                            else
                              ...
                              verifyKes(msgBytes, sig.kesProductSig.toBytes, sig.peerId, sig.kesTreeStep).flatMap { kesOk =>
                                if (!kesOk)
                                  Async[F].pure(
                                    Left(s"signer[$idx] peerId=${sig.peerId.value.value.take(16)}... KES sig verify failed")
                                      : Either[String, Unit]
                                  )
                                else
                                  ...
                                  Async[F].pure(verifyVrfStructural(sig.vrfProof.toBytes)).map { vrfOk =>
                                    if (!vrfOk)
                                      Left(
                                        s"signer[$idx] peerId=${sig.peerId.value.value.take(16)}... VRF proof structural check failed for shardId=${checkpoint.shardId} epoch=${checkpoint.epoch.value}"
                                      ): Either[String, Unit]
                                    else
        ...
        /** Re-exec path (T_depth1 only). For each MG in `checkpoint.includedSnapshots`, run `reExecuteDerivation` against the head
          * binary and compare the recomputed hash against the committee-signed `perMetagraphMptRoots(mg)`. Empty
          * `includedSnapshots` is a valid empty-window checkpoint (T_alive liveness ping per §6.1) — there's nothing to re-exec, so
          * acceptance reduces to the pre-check + finality phase combination above.
          *
          * All-match ⇒ `Accepted`. Any-mismatch ⇒ `RejectedReExecutionMismatch` with the entire signer list as the slash target
          * (§10.2 — every signer attested to the same wrong-derivation result; all of them deviated). The first mismatch carries
          * the reason; we don't short-circuit at the first mismatch because diagnostic completeness (which MGs diverged) is more
          * valuable than the trivial CPU saved by early-exit on a path that fires under degraded liveness only.
          */
        ...
        /** Verify Ed25519 over `msgBytes` using the signer's long-term VK recovered from the `PeerId`. Mirrors
          * [[io.constellationnetwork.node.shared.domain.nakamoto.MetagraphCommitteeGate]] receiver path lines 485-487:
          * `senderPubKey ← att.senderPeerId.value.toPublicKey[F]; Signing.verifySignature[F](msgBytes, sig)(senderPubKey)`.
          *
        ...
        /** Verify the KES product sig under the signer's registered master VK at the wire-carried `kesTreeStep`. Mirrors
          * [[io.constellationnetwork.dag.l0.infrastructure.snapshot.nakamoto.KesGossipVerification.verifyAttestation]] minus the
          * eta-period derivation: the design doc §3.1 puts `kesTreeStep` on the wire (CommitteeMemberSignature field) so the
          * receiver can verify non-interactively without re-deriving global eta period.
          *
          * '''Registry-absent carve-out''': if `kesRegistry.getKesVk(peerId)` returns `None`, accept on the strength of the Ed25519
          * signature alone (already verified). Mirrors the same carve-out in `KesGossipVerification` (lines 60-66 of that file) —
          * Slice 10 runtime registration will eventually cover all operators; the carve-out tolerates the bootstrap window.
          *
          * '''Lookup-first ordering''': the registry lookup runs BEFORE the sig decode. Rationale: when no registry entry exists, we
          * accept on the Ed25519 strength alone — we never need to decode the KES bytes. Putting decode first would reject the
          * Slice 10 bootstrap window (where genesis-only operators might gossip placeholder KES bytes), defeating the carve-out.
          * Empty sig bytes still reject upfront — those are a wire-shape violation regardless of registry state.
          */
        ...
        /** Structural VRF proof check — non-empty bytes at the EcVrf25519 minimum proof length (80 bytes per the EcVrf25519 wire
          * format). This is the Slice 9 placeholder; Slice 13's GSAM rewire will swap in the full
          * `CommitteeSortition.verifyMembership` check once the VRF VK registry is wired through the constructor.
          *
          * Why a method on the manager rather than a static helper: the production replacement will need access to
          * `Hasher[F]` + the VRF VK registry, both already in scope at the manager level. Keeping this as a method now means
          * the slice-13 swap touches one method body instead of unwinding helper plumbing.
          */
