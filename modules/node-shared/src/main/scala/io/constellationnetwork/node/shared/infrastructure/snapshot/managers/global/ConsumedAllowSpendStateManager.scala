...
/** A cross-shard consume candidate enumerated from the gl0 fold's `acceptedSpendActions`. Carries the spend's `allowSpendRef` (= the
  * content `Hash` of the consumed allow-spend — the single-use I-ONCE identity), the OWNER metagraph `M` (`currencyId`, the metagraph whose
  * ledger holds the reserved funds), the PRODUCING metagraph `M′` (the `acceptedSpendActions` key — the shard that processed the spend),
  * plus the `source`/`destination`/`amount` the settlement moves. A consume is cross-shard iff `shardIdFor(M) ≠ shardIdFor(M′)`.
  */
...
/** Result of the W3d nullifier pass over the cross-shard consume candidates: the new spent-set markers to write this ordinal (one per
  * ACCEPTED cross-shard consume), and the set of allow-spend hashes that were REJECTED (failed the include or absence check —
  * double-consume / no-such-reservation). The markers are keyed by `allowSpendHash` (the consumedAllowSpends key identity).
  */
...
/** gl0-side ATOMIC cross-shard allow-spend settlement ("I-ONCE", Option 2: read-side effective-balance overlay).
  ...
  * EMPTY at `numShards = 1`), so it is tiny in practice and the effective view is DERIVED (rebuildable from the committed spent-set), NOT
  * stored and NOT in any consensus root.
  *
  * '''Determinism.''' Every iteration is over `SortedMap`/`SortedSet`; same-snapshot collisions (two shards consuming one `AS`) are
  * resolved by processing candidates in `allowSpendHash`-sorted order — the first passes the absence check, settles + marks; the rest fail
  * absence → rejected. All hashing routes through `Hasher[F]` (via `ShardAssignment.shardIdFor`). No clock / env. Every honest node
  * computes byte-identical post-state.
  *
...
trait ConsumedAllowSpendStateManager[F[_]] {
  ...
  /** Materialize the full `Option[contract] → source → active-allow-spend-set` mirror (the `ActiveAllowSpends` partition, fieldId 7) — the
    * include-check input for [[settleCrossShardConsumes]]. Identical to `AllowSpendStateManager.materializeActiveAllowSpendsFromMpt`;
    * exposed here so the cross-shard handler is self-contained over a single reader.
    */
  ...
  /** W3d nullifier pass. For each cross-shard consume (processed in `allowSpendHash`-sorted order):
    ...
    *   - REJECT if `AS` is absent from `M`'s active-allow-spend mirror (`lastActiveAllowSpends` under the `M` scope, matched by `hash(AS)`)
    *     — no such reservation.
