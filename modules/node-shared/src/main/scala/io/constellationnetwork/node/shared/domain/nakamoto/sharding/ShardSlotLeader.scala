...
/** Per-shard slot leader election within a shard committee (hierarchical-shard-checkpoints v1, §5.3 + §5.6).
  ...
  *
  *   1. '''Eta is shard-scoped.''' We do NOT feed gl0's `eta` straight in; instead we derive
  *      `shardEta = Hasher.hash(ShardEtaInput("shard-eta", shardId, gl0Eta))`. Domain-separated so a per-shard leader VRF can't leak gl0
  *      leader eligibility and so two shards electing at the same slot+gl0-eta produce independent draws (§5.6).
  *   2. '''Relative stake is in-committee.''' σ_i is the operator's stake share within the committee (uniform `1 / K_S` per v1 stable-σ
  *      rule from `[[project-216-committee-stake-drift-fix]]`), NOT their share of the whole gl0 active set.
  *
  * '''Determinism.''' Everything routes through `Hasher[F]` for hashing and `EligibilityChecker` for threshold arithmetic — no
  * hand-rolled Blake2b, no Double, no second serialization surface. Two honest nodes given the same
  * `(shardId, gl0Eta, slot, slotGap, σ, lddConfig)` produce byte-equivalent VRF inputs and byte-equivalent thresholds.
  *
  ...
  * nothing more.
  *
  * '''Why `slotGap` is in the signature.''' The design doc §5.3 reference snippet passes `slotGap` to `checkEligibility`, but the
  * slice 7 task's `isLeader` sketch omits it. LDD's threshold is gap-conditioned (the snowplow ramp drives `f(δ)`); omitting `slotGap`
  * would change the protocol. We accept it explicitly so each call site can pass `currentSlot - lastShardTipSlot` (the shard-chain
  * analogue of the gl0 slotGap), and document the deviation here rather than silently fixing it inside the wrapper.
  */
trait ShardSlotLeader[F[_]] {
  ...
  /** Compute `shardEta` for `(shardId, gl0Eta)`. Hash routes through `Hasher[F]`; result is the 32 raw SHA-256 digest bytes of the
    * canonical JSON encoding of [[ShardSlotLeader.ShardEtaInput]] — matching the byte shape `EligibilityChecker.checkEligibility`
    * requires for its `eta` argument (`vrfProofForSlot` asserts `eta.length == 32`).
    *
  ...
  /** Per-slot leader eligibility check. Returns `Some(proof, vrfOutput)` if this operator wins the slot lottery for
    * `(shardEta, slot, σ_in_committee)`, `None` otherwise.
    *
    * Delegates threshold arithmetic to [[EligibilityChecker.checkEligibility]] verbatim — same LDD snowplow `1 - (1 - f(δ))^σ`,
    * same `Ratio` arithmetic, same byte-equivalent behavior across JVMs.
    */
  def isLeader(
    vrfSk: Array[Byte],
    shardEta: Array[Byte],
  ...
  /** Verifier path. Given a claimed leader's VRF proof, confirms both (a) the proof cryptographically verifies under `vrfVk` for the
    * canonical `(shardEta, slot)` message AND (b) the VRF output falls below the LDD threshold for the gap-conditioned
    * `(σ_in_committee, lddConfig)`. Delegates to [[EligibilityChecker.verifyEligibility]] verbatim.
    */
  def verifyLeader(
    vrfVk: Array[Byte],
    shardEta: Array[Byte],
...
object ShardSlotLeader {
  ...
  /** Domain-separation tag carried in-band on the encoded `ShardEtaInput`. Must NOT collide with any other VRF or eta-derivation tag in
    * the codebase — keeps per-shard leader VRF eta independent of `CommitteeSortition`'s `"committee"` tag and of gl0's leader-VRF
  ...
  /** Canonical hash input for the per-shard eta derivation. The `tag` field provides domain-separation; `shardId` and `gl0Eta` are the
    * two pieces of per-shard identity. Encoded via the standard `Hasher[F]` surface so the resulting bytes are byte-equivalent across
    * the JVM-side and any cross-implementation verifier (e.g. the Go sidecar).
    *
  ...
  /** Build a [[ShardSlotLeader]] backed by the supplied [[EligibilityChecker]]. Callers MUST pass the same `EligibilityChecker[F]`
    * instance the rest of the consensus path is using (so the per-shard slot-leader threshold arithmetic is bit-for-bit identical with
    * the gl0 path — same `Log1p`/`Exp` precision and same Bifrost continued-fraction interpreters).
    *
