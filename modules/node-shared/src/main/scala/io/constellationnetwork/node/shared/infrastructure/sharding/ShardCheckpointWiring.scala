...
/** Production-wiring helper for the hierarchical-shard-checkpoints v1 ACCEPTANCE side (priority 1 of the production-wiring slice).
  ...
  * `processCurrencySnapshots` pipeline, which is not available at the GSAM-construction point (GSAM is being constructed). v1 therefore
  * takes the derivation closure as a parameter so each call site passes what it has; both current call sites pass [[noReExecDerivation]] (a
  * documented v1 stub — see its scaladoc for the safety analysis). The full re-exec wiring is a follow-up coupled with the producer side.
  *
...
object ShardCheckpointWiring {
  ...
  /** v1 stub for the `T_depth1_shard` re-exec derivation. Returns a fixed sentinel hash for every `(metagraphAddress, headBinary)`.
    *
    ...
    *     does NOT cause a false slash in v1: `GlobalSnapshotAcceptanceManager.processShardCheckpoints` only LOGS the rejected-mismatch
    *     signer list (Slice 13) — slashing-evidence emission is Slice 16/17 territory and is not wired into the accept loop yet. So the
    *     worst case is "degraded-shard non-quorum checkpoints are not admitted until quorum returns", never "honest signers slashed".
    *
    * The real derivation (re-run `processCurrencySnapshots` and compare) is the follow-up that lands with the producer side.
    */
  def noReExecDerivation[F[_]: Async]: (Address, Signed[StateChannelSnapshotBinary]) => F[Hash] =
    (_: Address, _: Signed[StateChannelSnapshotBinary]) => Async[F].pure(Hash.empty)
