...
/** Event-sourced MPT state with checkpoint+replay for fork handling.
  *
  * Adapted from Bifrost's EventSourcedState pattern. Since tessellation's MPT is forward-only
  * (no unapply), we use checkpoint+replay instead of apply/unapply:
  *
  ...
  *
  * This avoids the need for undo logs or content-addressed trie storage while still supporting
  * fork switches correctly. The cost of a reorg = replay from last checkpoint, which is bounded
  * by the finality depth (k blocks max).
  *
  * Thread safety: all state mutations go through a Semaphore (shared with SnapshotLeaderLoop
  * and NakamotoSyncDaemon).
  */
...
object EventSourcedMptState {
  ...
  def make[F[_]: Async](
    ...
    } yield new EventSourcedMptState[F] {
