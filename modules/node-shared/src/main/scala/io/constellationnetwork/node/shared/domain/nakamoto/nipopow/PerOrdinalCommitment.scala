...
/** Per-ordinal NIPoPoW super-level — the `towerEligibility` slot of [[PerOrdinalCommitment]].
  *
  * Sourced (when wired) from the EXISTING tower computation: the highest super-level µ a finalized snapshot passed
...
/** §3 NIPoPoW — the typed tuple committed, one per finalized snapshot ordinal, as a leaf of the historical-commitment SMT
  * ([[HistoricalCommitmentSmtStore]]) whose single root is anchored as `smtRoot` in the gl0 `GlobalSnapshotStateProof`.
  *
  * All three artifacts are DETERMINISTIC functions of a single finalized ordinal `i` (so producer and follower derive byte-identical
  ...
  *     references descend strictly toward genesis (well-founded, like normal block-hash chaining).
  ...
  * hand-rolled). One inclusion proof against `smtRoot(N)` then reveals this whole tuple for any finalized `i ≤ N−k`.
  */
@derive(encoder, decoder, eqv, show)
