...
/** Per-operator COOLDOWN gate over the `Slashings` (fieldId 34) partition — the committee-EXCLUSION half of the watchtower slash tier
  ...
  * [[io.constellationnetwork.node.shared.infrastructure.sharding.ShardCheckpointWiring.committeeFor]] so a slashed operator is ABSENT from
  * the shard-committee draw (and therefore its checkpoint signature can never pass the committee-membership pre-check, i.e. can never count
  * toward `kQuorum`) until its cooldown elapses.
  *
  ...
  * verdicts for the same embedded checkpoint ⇒ divergent adopt decisions ⇒ fork. The committee is memoized per `(shardId, epoch)` and MUST
  * stay a pure function of that key. So the exclusion is pinned per EPOCH, Cardano-style (the settled epoch-staggering design: "the signer
  * set for epoch N is a function of state settled by the end of epoch N−2"):
  *
  *   - '''Anchor.''' [[slashAnchorOrdinal]]`(E) = (E−1)·R − 1` — the LAST gl0 ordinal of eta-period `E−2` (`R =
  *     nakamoto.etaRotationSnapshots`, the same period length the committee's eta resolver keys on). Only slash records written at-or-below
  *     the anchor participate in epoch `E`'s exclusion.
  *   - '''Why the reads are uniform from a live base store.''' The partition is APPEND-ONLY (entries are written once by the GSAM accept
  *     fold, never mutated, never pruned; the S01/S02 preservation keeps them across every GSI rebuild), and each entry's
  *     `eventOrdinal`/`cooldownUntilEpoch` are immutable consensus-written fields. Filtering `eventOrdinal <= anchor(E)` therefore
  *     reconstructs EXACTLY the set of records the canonical chain had written by the anchor, on ANY node whose finalized base has reached
  *     the anchor — the same per-key-immutability discipline that makes the committee's `etaForEpoch` read (the `HistoricalStakeSnapshot`
  *     lookup off the SAME `mptStore`) uniform. The anchor sits a full eta-period (+1 boundary) below any ordinal at which an epoch-`E`
  *     committee is legitimately evaluated — deeper than the k₁ write-freeze floor — so the filtered prefix is in the write-frozen common
  *     prefix of every honest node.
  *   - '''Margin.''' The earliest legitimate epoch-`E` draw is the pseudo-predictability point (~2/3 into period `E−1`, ordinal `≈ E·R −
  *     R/3`); a node evaluating there has finalized base `≥ E·R − R/3 − k₁ ≈ E·R − 2.06·k₁`, which exceeds `anchor(E) ≈ E·R − 3.03·k₁` by
  *     `≈ k₁` (R = round(3.03·k₁)).
  ...
  *     `anchorSettled = false` and the caller MUST NOT memoize it (`ShardCheckpointWiring` only caches settled draws) — a fresh evaluation
  *     at real use time (anchor long settled) is exact. This mirrors `EtaStateManager`'s discipline of not caching the empty-chain-walk
  *     fallback.
  *
  ...
  * .applySlash`), while the committee draw has only the eta-period/ordinal axis: under the Nakamoto overlay the two are the SAME axis — the
  * leader loop produces EVERY snapshot with `TimeTrigger` (`SnapshotLeaderLoop`), so `epochProgress` advances exactly 1 per ordinal from
  * genesis (both start at 0) and the consensus EpochProgress AT the anchor ordinal IS the anchor ordinal. Sharding (`numShards > 1`) is a
  * Nakamoto-overlay-only mechanism, so this equivalence holds wherever this reader is consulted; it is asserted by
  ...
  *
  * Exclusion must never make `kQuorum` structurally unmeetable (the committee is a subset of the post-exclusion pool). Polkadot disables
  * offenders only up to a limit and lets NEW offenders escape disabling once the limit is reached; [[effectiveExclusion]] adapts exactly
  * that: candidates are ordered oldest-slash-first (`(minEventOrdinal, peerId)` — deterministic), and at most `max(0, |active| − kQuorum)`
  * of them are excluded, so at least `kQuorum` eligible validators always remain (liveness floor = the existing cluster-uniform
  * `nakamoto.committee.kQuorum`; no new tunable). Everyone-slashed with `|active| <= kQuorum` ⇒ nobody is excluded (liveness over
  * exclusion, loudly visible in the slash ledger either way).
  */
trait SlashCooldownReader[F[_]] {

  /** The epoch-pinned exclusion view for `epoch`: the deterministic per-peer cooldown candidates (already filtered to records active at the
    * epoch's anchor) and whether this node's base has settled the anchor (only then may the caller memoize a committee drawn from it).
    */
...
object SlashCooldownReader {
  ...
  /** Exclusion candidates for one epoch.
    ...
    *   cooldownUntilEpoch`), sorted oldest-slash-first then by `peerId` — the deterministic priority order [[effectiveExclusion]] consumes.
    * @param anchorSettled
    *   `true` iff this node's finalized base has reached the epoch's anchor ordinal (or the anchor is pre-genesis), i.e. the candidate list
    *   is FINAL for this epoch on every honest node. `false` ⇒ the caller must not cache any committee derived from it.
    */
  ...
  /** The epoch's slash anchor: the last gl0 ordinal of eta-period `epoch − 2`, i.e. `(epoch − 1)·R − 1`. Negative for the bootstrap epochs
    * (`epoch <= 1`, where `R ≥ 1`) ⇒ no record can qualify ⇒ exclusion structurally empty — mirroring the periods-0/1 bootstrap-eta
    * treatment.
    */
  ...
  /** PURE candidate derivation: records ACTIVE at `anchorOrdinal` (`eventOrdinal <= anchor < cooldownUntilEpoch`, both fields
    ...
    * `(minEventOrdinal, peerId)` — the deterministic oldest-slash-first priority. Records younger than the anchor belong to a LATER epoch's
    * view; expired cooldowns (`cooldownUntilEpoch <= anchor`) have served their term.
    */
  def activeSlashCandidates(entries: Iterable[SlashedRegistryEntry], anchorOrdinal: Long): List[(PeerId, Long)] =
    entries.iterator.filter { e =>
  ...
  /** Production reader over the SAME finalized-base `MptStore` the committee draw's eta resolver reads (`SharedServices`
    ...
    * `Slashings` insert lands in once finalized). Prefix-scans fieldId 34 with the canonical [[InvalidStateProofSlashedReader.entryCodec]]
    * and reports `anchorSettled` off `lastPersistedOrdinal` (see class scaladoc for why the anchor filter makes the live-store scan
    * epoch-pure).
    *
