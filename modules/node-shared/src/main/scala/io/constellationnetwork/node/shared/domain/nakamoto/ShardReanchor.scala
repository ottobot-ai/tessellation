...
object ShardReanchor {
  ...
  /** Classify a checkpoint window `nel` (oldest-first) for one metagraph against gl0's committed SC tip.
    ...
    *   - `Reanchor(idx)`: `tipHash` is continued by NO binary (orphaned), but the window is genesis-rooted (head parent === `Hash.empty`)
    *     and reaches at-or-past `tipOrdinal`. Because the committee chain-link-validated the window, a genesis-rooted window's binary index
    *     EQUALS its metagraph ordinal, so the canonical successor of the dead tip is at index `tipOrdinal + 1` (clamped to the window
    *     head's last index for the lateral-only case where the reorg winner exists at `tipOrdinal` but its successor has not landed yet).
