    ...
    def enqueue(snapshot: Signed[S], snapshotInfo: C)(implicit hasher: Hasher[F]) =
      snapshot.value.hash.flatMap { hash =>
        hashCache(hash).set(snapshot.some) >>
          ...
          // Narrow error handling to just the info write, not the entire chain
          snapshotInfoLocalFileSystemStorage
            ...
            .handleErrorWith { e =>
              ...
              snapshotInfoLocalFileSystemStorage.exists(snapshot.ordinal).flatMap { exists =>
                if (exists)
                  logger.info(s"Snapshot info already exists on disk for ordinal=${snapshot.ordinal}")
                else
                  // Re-raise error so caller can retry. On retry, snapshotExists check
                  // will pass (snapshot already written), and we'll attempt info write again.
