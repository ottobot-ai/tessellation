 ...
 *
 * Multiple subsystems can pause/resume production. The gate is open (production allowed)
 * only when ALL registered pause reasons are cleared. This prevents:
    ...
    Ref.of[F, Set[String]](Set.empty).map { reasonsRef =>
      new ProductionGate[F] {
        ...
        def resume(reason: String): F[Unit] =
          ...
          }.flatMap { case (wasActive, nowOpen) =>
